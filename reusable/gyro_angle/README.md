...
V8 在综合测试应用层允许自动恢复；`GyroTurn` 本身仍以 FAULT 表示当前动作失败，不能将它误当 DONE。

- `MpuYaw_Task` 在故障后一秒重试总线/传感器，保留已经完成的零偏标定；没有完成过标定则继续等待实际静止。新配置完成仍须收到新样本才 READY。
- 每次后台重启递增 `MpuYawReading.generation`，丢失区间不积分为已知角度。GyroTurn 拒绝跨 generation 的旧目标；其他 yaw 消费者也必须撤销旧快照/路线。
- 综合应用先记录故障并取消旧动作，再在最长 800 ms 自动等待后释放底盘、清故障、恢复搜索；遥控 STOP 禁止这一流程。请按目标应用的所有权规则接入，不在任意 FAULT 循环中直接 Start。
- 绕障包装层可在新动作入口选择编码器估计转角；陀螺仪失败后的 10 秒内优先降级。运行中不切换角度坐标，单独复用 GyroTurn 不会自动获得这一包装层策略。
- 导出包的绕障包装层测试另需 wheel_encoder、line_search_model、vehicle_geometry 头文件；它们随包提供。未测实车，不将编码器估计标成实测 yaw。

## V7 调度与恢复约定（历史背景，故障重试以 V8 为准）
