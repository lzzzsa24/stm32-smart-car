...
static void test_recognition_speed_cap(void)
{
  ...
  Promoted_line_tracking_reset(); reset(); Promoted_line_tracking_set_no_line_forward(0);
  DriveBase_SetSpeedLimitCps(1200);
  for (ms = 0; ms < 800; ++ms)
  {
    for (i = 0; i < 4; ++i) counts[i] += pins[i]>0 ? 2 : (pins[i]<0 ? -2 : 0);
    ++tick; DriveBase_Task(tick);
    Promoted_line_tracking_compute(&reading, 3000, &output);
    Promoted_line_tracking_apply_command(&output, 3599);
    DriveBase_GetTelemetry(&t);
  ...
  DriveBase_SetSpeedLimitCps(0);
  ++tick; Promoted_line_tracking_compute(&reading,3000,&output);
  DriveBase_GetTelemetry(&t);
  assert(t.requested_cps[0] == -LINE_SEARCH_TARGET_CPS);
  Promoted_line_tracking_reset(); reset();
...
{
  Promoted_LineTrackingReading reading={1,0,0,0};
  Promoted_LineTrackingCommand output;
  DriveBaseTelemetry t;
  int32_t delta[4]={1,28,105,105};
  unsigned i;
  Promoted_line_tracking_reset(); reset();
  Promoted_line_tracking_set_smooth_mode(0);
  for(i=0;i<30;++i)
  {
    sample(delta);
    Promoted_line_tracking_compute(&reading,3000,&output);
...
{
  Promoted_LineTrackingCommand out;
  ...
  Promoted_line_tracking_reset(); reset();
  Promoted_line_tracking_set_no_line_forward(1); Promoted_line_tracking_set_smooth_mode(1);
  Promoted_line_tracking_set_turn_gain_percent(200);
  advanced_set_forward_speed_limit(2200);
  for(i=0;i<50;++i)
  {
    Promoted_line_tracking_compute(&r,3000,&out);
    assert(out.valid && out.left_cps>0 && out.right_cps>0);
    if(fixed) Promoted_line_tracking_apply_command(&out,2200);
...
static void test_integrated_line_cap(void)
{
  ...
  Promoted_LineTrackingCommand command={2500,2000,Promoted_LINE_ACTION_LEFT_ADJUST,1};
  int32_t zero[4]={0};
  ...
  Promoted_line_tracking_apply_command(&command,0); sample(zero);
  DriveBase_GetTelemetry(&current); assert(current.mode==DRIVE_BASE_STOPPED);
  Promoted_line_tracking_apply_command(&command,3000);
  { int32_t moving[4]={30,30,30,30}; sample(moving); }
  DriveBase_Stop(DRIVE_STOP_BRAKE); Promoted_line_tracking_apply_command(&command,2200);
  DriveBase_GetTelemetry(&current); assert(current.mode==DRIVE_BASE_BRAKING);
  command.valid=0; Promoted_line_tracking_apply_command(&command,0);
  DriveBase_GetTelemetry(&current); assert(current.mode==DRIVE_BASE_BRAKING);
  DriveBase_Stop(DRIVE_STOP_COAST); Promoted_line_tracking_reset();
  Promoted_line_tracking_set_turn_gain_percent(100);
...
static void test_bounded_automatic_waits(void)
{
  ...
  Promoted_line_tracking_reset(); reset(); LineWaitGuard_Reset(&guard);
  for(ms=0;ms<10000;++ms)
  {
    ...
    { ++begins; DriveBase_Stop(DRIVE_STOP_COAST); DriveBase_ClearFault(); Promoted_line_tracking_reset(); }
  ...
  Promoted_line_tracking_reset(); reset(); command(2500,2500,0);
  for(ms=0;ms<100;++ms) sample(zero);
  assert(DriveBase_GetFaultMask()!=0);
  DriveBase_ClearFault(); Promoted_line_tracking_reset(); LineWaitGuard_Drive(1);
  DriveBase_GetTelemetry(&t);
...
{
  Promoted_LineTrackingReading reading;
  Promoted_LineTrackingCommand output;
  DriveBaseTelemetry t;
  unsigned ms,w,mask,phase_ms,reentries=0;
  int32_t previous_left=0;
  Promoted_line_tracking_reset(); reset();
  Promoted_line_tracking_set_no_line_forward(0);
  Promoted_line_tracking_set_smooth_mode(0);
  ...
  for(ms=0;ms<1608;++ms)
  {
    ...
    reading=(Promoted_LineTrackingReading){mask&1,(mask>>1)&1,(mask>>2)&1,(mask>>3)&1};
    Promoted_line_tracking_compute(&reading,3000,&output);
...
static void test_real_ambiguous_inner_handoffs(unsigned first_right)
{
  ...
  for(ms=0;ms<2600;++ms)
  {
    ...
    if(ms>=200 && (ms-200)%200>=70)
    {
      ...
      assert(Promoted_LineRecovery_IsSearching());
...
static void test_real_broad_corner_directions(unsigned overlapping)
{
  ...
  unsigned ms, w;
  Promoted_line_tracking_reset(); reset(); Promoted_line_tracking_set_no_line_forward(0);
  Promoted_line_tracking_set_smooth_mode(1);
  for(ms=0;ms<5600;++ms)
  {
    ...
    Promoted_LineTrackingReading reading={mask&1,(mask>>1)&1,(mask>>2)&1,(mask>>3)&1};
    for(w=0;w<4;++w) counts[w]+=pins[w]>0?3:(pins[w]<0?-3:0);
    ++tick; DriveBase_Task(tick);
    Promoted_line_tracking_compute(&reading,3000,&output);
    Promoted_line_tracking_apply_command(&output,MOTOR_PWM_PERIOD);
...
static void test_real_search_capture(void)
{
  DriveBaseTelemetry t;
  Promoted_LineTrackingCommand output={0};
  Promoted_LineTrackingReading reading={1,0,1,0};
  ...
  assert(!DriveBase_GetFaultMask());
  Promoted_line_tracking_reset(); DriveBase_Stop(DRIVE_STOP_COAST);
...
static void test_real_exit_direction_correction(void)
{
  ...
  {
    Promoted_LineTrackingCommand out={0}; DriveBaseTelemetry t;
    Promoted_line_tracking_reset(); reset(); Promoted_line_tracking_set_no_line_forward(0);
    for(ms=0;ms<1000;++ms)
    {
      unsigned mask=ms<100?(side?8:2):0;
      Promoted_LineTrackingReading r;
      if(ms==300 && middle) mask=5;
      if(ms==312) mask=side?2:8;
      for(w=0;w<4;++w) counts[w]+=pins[w]>0?3:(pins[w]<0?-3:0);
      ++tick; DriveBase_Task(tick);
      r=(Promoted_LineTrackingReading){mask&1,(mask>>1)&1,(mask>>2)&1,(mask>>3)&1};
      Promoted_line_tracking_compute(&r,3000,&out);
      ...
      if(ms>=313)
      {
        assert(t.mode==DRIVE_BASE_SPEED && Promoted_LineRecovery_IsSearching());
...
static void test_real_strong_exit_handoff(void)
{
  ...
  for(right=0;right<2;++right) for(overlap=0;overlap<2;++overlap)
  {
    ...
    for(ms=0;ms<700;++ms)
    {
      ...
      r=(Promoted_LineTrackingReading){mask&1,(mask>>1)&1,(mask>>2)&1,(mask>>3)&1};
      Promoted_line_tracking_compute(&r,3000,&out);
      Promoted_line_tracking_apply_command(&out,MOTOR_PWM_PERIOD);
      DriveBase_GetTelemetry(&t);
      assert(!t.fault_mask && t.mode==DRIVE_BASE_SPEED);
...
static void test_slow_outer_profiles(void)
{
  ...
  for(smooth=0;smooth<2;++smooth) for(active=0;active<2;++active)
  {
    Promoted_LineTrackingCommand out={0}; DriveBaseTelemetry t;
    Promoted_line_tracking_reset(); reset(); Promoted_line_tracking_set_no_line_forward(0);
    Promoted_line_tracking_set_smooth_mode((uint8_t)smooth);
    Promoted_line_tracking_set_turn_gain_percent((uint16_t)gain);
    if(active)
    {
      Promoted_LineTrackingReading white={0};
      tick+=100; Promoted_line_tracking_compute(&white,3000,&out);
      assert(Promoted_LineRecovery_IsSearching());
    }
    for(stage=0;stage<4;++stage) for(ms=0;ms<250;++ms)
    {
      unsigned mask=stage==0?(right?8:2):(stage==1?(right?12:3):(stage==2?5:15));
      Promoted_LineTrackingReading r={mask&1,(mask>>1)&1,(mask>>2)&1,(mask>>3)&1};
      for(w=0;w<4;++w) counts[w]+=pins[w]>0?1:(pins[w]<0?-1:0);
      ++tick; DriveBase_Task(tick);
      Promoted_line_tracking_compute(&r,3000,&out);
      Promoted_line_tracking_apply_command(&out,MOTOR_PWM_PERIOD); DriveBase_GetTelemetry(&t);
  ...
  Promoted_line_tracking_reset(); reset(); Promoted_line_tracking_set_turn_gain_percent(100);
...
static void test_real_visible_arc_and_loss(unsigned right)
{
  ...
  for(ms=0;ms<1500;++ms)
  {
    ...
    for(w=0;w<4;++w) counts[w]+=pins[w]>0?3:(pins[w]<0?-3:0);
    ++tick; DriveBase_Task(tick);
    Promoted_line_tracking_compute(&r,3000,&out); Promoted_line_tracking_apply_command(&out,MOTOR_PWM_PERIOD);
  ...
  assert(!BuzzerPhrase400_IsPlaying());
  Promoted_line_tracking_reset(); for(w=0;w<4;++w) assert(pins[w]==0);
...
static void test_observe_faults(void)
{
  ...
  {
    Promoted_LineTrackingReading reading={0}; Promoted_LineTrackingCommand out;
    Promoted_line_tracking_reset(); reset(); Promoted_line_tracking_set_no_line_forward(0);
    for(i=0;i<300;++i)
    {
      int32_t delta[4]={0,pins[1]>0?50:(pins[1]<0?-50:0),
          pins[2]>0?50:(pins[2]<0?-50:0),pins[3]>0?50:(pins[3]<0?-50:0)};
      sample(delta); Promoted_line_tracking_compute(&reading,3000,&out);
      if(out.valid) DriveBase_SetSideCps(out.left_cps,out.right_cps);
    }
...
static void test_mode12_shared_following(void)
{
  ...
  static Promoted_LineTrackingAction actions[1800];
  ...
  for(capped=0;capped<2;++capped) for(shared=0;shared<2;++shared)
  {
    ...
    Promoted_LineTrackingCommand out={0}; DriveBaseTelemetry drive;
    Promoted_line_tracking_reset(); reset(); tick=1000;
    Promoted_line_tracking_set_no_line_forward(1); Promoted_line_tracking_set_smooth_mode(0);
    Promoted_line_tracking_set_turn_gain_percent(200); /* stale old KEY1 profile */
    if(shared) Promoted_line_tracking_start_following();
    else
    {
      Promoted_line_tracking_reset(); Promoted_line_tracking_set_no_line_forward(0);
      Promoted_line_tracking_set_smooth_mode(1); Promoted_line_tracking_set_turn_gain_percent(100);
    }
    for(ms=0;ms<1800;++ms)
    {
      ...
      if(shared) action=Promoted_line_tracking_follow_once(3000,cap);
      else
      {
        Promoted_LineTrackingReading r=Promoted_line_tracking_read();
        action=Promoted_line_tracking_compute(&r,3000,&out);
        Promoted_line_tracking_apply_command(&out,cap);
      }
      DriveBase_GetTelemetry(&drive);
    ...
    Promoted_line_tracking_reset(); line_gpio_mask=0; ++tick;
    Promoted_line_tracking_follow_once(3000,cap); DriveBase_GetTelemetry(&drive);
    assert(drive.requested_cps[0]==-LINE_SEARCH_TARGET_CPS && drive.requested_cps[2]==LINE_SEARCH_TARGET_CPS);
    Promoted_line_tracking_reset(); for(w=0;w<4;++w) assert(!pins[w]);
  }
...
static void test_mode1_boost_real_drive(void)
{
  unsigned ms,w;
  DriveBaseTelemetry d;
  reset(); Promoted_line_tracking_start_following(); line_gpio_mask=5;
  for(ms=0;ms<1000;ms+=10)
  {
    tick+=10; Promoted_line_tracking_set_straight_boost(1);
    Promoted_line_tracking_follow_once(3000,MOTOR_PWM_PERIOD);
  }
  DriveBase_GetTelemetry(&d);
  assert(DriveBase_EquivalentCpsFromPwm(2700)==3815);
  for(w=0;w<4;++w) assert(d.requested_cps[w]==4544);
  Promoted_line_tracking_follow_once(3000,2600); DriveBase_GetTelemetry(&d);
  for(w=0;w<4;++w) assert(d.requested_cps[w]==DriveBase_EquivalentCpsFromPwm(2600));
  line_gpio_mask=8; ++tick; Promoted_line_tracking_follow_once(3000,MOTOR_PWM_PERIOD);
  DriveBase_GetTelemetry(&d);
  assert(d.requested_cps[0]==2200 && d.requested_cps[1]==2200);
  assert(d.requested_cps[2]==0 && d.requested_cps[3]==0);
  Promoted_line_tracking_follow_once(3000,0); DriveBase_GetTelemetry(&d);
  assert(d.mode==DRIVE_BASE_STOPPED);
  for(w=0;w<4;++w) assert(!d.requested_cps[w] && !pins[w]);
  Promoted_line_tracking_start_following(); line_gpio_mask=5;
  for(ms=0;ms<1000;ms+=10) { tick+=10; Promoted_line_tracking_follow_once(3000,MOTOR_PWM_PERIOD); }
  DriveBase_GetTelemetry(&d);
...
static void test_bypass_contact_handoff(void)
{
  ...
  {
    int32_t side=i ? 1 : -1;
    reset(); Promoted_line_tracking_rejoin_from_bypass(masks[i]);
    /* The LED pulse is already over at application handoff. */
    line_gpio_mask=0; ++tick; Promoted_line_tracking_follow_once(3000,MOTOR_PWM_PERIOD);
    DriveBase_GetTelemetry(&d);
    assert(d.requested_cps[0]==side*LINE_SEARCH_TARGET_CPS);
    assert(d.requested_cps[2]==-side*LINE_SEARCH_TARGET_CPS);
    Promoted_line_tracking_rejoin_from_bypass(masks[i]);
    line_gpio_mask=i ? 8U : 2U; ++tick; Promoted_line_tracking_follow_once(3000,MOTOR_PWM_PERIOD);
    DriveBase_GetTelemetry(&d);
    assert(d.requested_cps[i ? 0 : 2]==2200);
    assert(d.requested_cps[i ? 2 : 0]==0);
    Promoted_line_tracking_reset(); line_gpio_mask=0;
  }
...
static void test_fast_turn_assist(void)
{
  ...
  reset(); Promoted_line_tracking_reset();
...
static void test_fast_follow_continuity(void)
{
  ...
  reset(); Promoted_line_tracking_start_following(); Promoted_line_tracking_set_fast_follow(1);
  for(pass=0;pass<10;++pass) for(phase=0;phase<5;++phase)
    for(frame=0;frame<20;++frame)
    {
      ...
      sample(delta); Promoted_line_tracking_compute(&r,3000,&out);
      Promoted_line_tracking_apply_command(&out,3599); DriveBase_GetTelemetry(&d);
  ...
  {
    Promoted_LineTrackingReading outer={0,1,0,0};
    Promoted_line_tracking_compute(&outer,3000,&out); Promoted_line_tracking_apply_command(&out,3599);
...
static void test_fast_forward_steering_no_pulse(void)
{
  ...
  for(side=0;side<2;++side)
  {
    Promoted_LineTrackingReading r={side?0:1,0,side?1:0,0};
    reset(); Promoted_line_tracking_start_following(); Promoted_line_tracking_set_fast_follow(1);
    for(ms=0;ms<160;++ms)
    {
      ++tick; DriveBase_Task(tick);
      Promoted_line_tracking_compute(&r,3000,&out); Promoted_line_tracking_apply_command(&out,3599);
      DriveBase_GetTelemetry(&d);
