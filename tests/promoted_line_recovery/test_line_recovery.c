...
static Promoted_LineTrackingCommand output;
...
static void sample(unsigned mask,uint32_t dt,int16_t base)
{
  ...
  Promoted_line_tracking_compute(&r,base,&output);
...
{
  Promoted_line_tracking_reset(); memset(&telemetry,0,sizeof telemetry); BuzzerPhrase400_Init();
  attacks=spins=reversals=brakes=0; previous_spin=0;
  Promoted_line_tracking_set_no_line_forward(forward); Promoted_line_tracking_set_smooth_mode(smooth);
}
static void test_fast_follow(void)
{
  ...
  for(side=0;side<2;++side)
  {
    ...
    hold(5,260); assert(output.left_cps==3600 && output.right_cps==3600);
    sample(pair,10,3000);
...
static void test_fast_no_timed_holds(void)
{
  ...
  for(side=0;side<2;++side)
  {
    ...
    reset(0,1); Promoted_line_tracking_set_fast_follow(1);
    sample(5,1,3000); assert(output.left_cps==3600);
    sample(5,20,3000); assert(output.left_cps==3600); /* no stationary acceleration hold */
    for(repeat=0;repeat<50;++repeat)
    {
      ...
      assert(output.valid && Promoted_LineRecovery_IsSearching() && output.left_cps==-output.right_cps);
      sample(side?1:4,0,3000); assert(Promoted_LineRecovery_IsSearching());
      sample(side?1:4,1,3000);
      assert(output.valid && !Promoted_LineRecovery_IsSearching());
  ...
  tick=UINT32_MAX-5; reset(0,1); Promoted_line_tracking_set_fast_follow(1);
  ...
  sample(5,1,3000); assert(output.valid && output.left_cps==3600);
...
static void test_fast_wide_cruise(void)
{
  ...
  for(n=0;n<5;++n)
  {
    ...
    sample(5,1,3000); assert(output.left_cps==3600 && output.right_cps==3600);
...
{
  assert(!output.valid && telemetry.mode==DRIVE_BASE_SPEED && Promoted_LineRecovery_IsSearching());
...
static void test_corner_edge_chatter(void)
{
  ...
  for(smooth=0;smooth<2;++smooth) for(side=0;side<2;++side)
  {
    ...
    for(i=0;i<100;++i)
    {
      ...
      assert(brakes==before && Promoted_LineRecovery_IsSearching());
    }
    for(i=0;i<20;++i)
    {
...
static void test_patterns_and_narrow_windows(void)
{
  ...
  for(mode=0;mode<5;++mode) for(i=0;i<sizeof wide/sizeof wide[0];++i)
  {
    ...
    hold(0,220); assert(!output.valid && Promoted_LineRecovery_IsSearching());
  }
  ...
  assert(output.valid && Promoted_LineRecovery_IsSearching()); /* Forward contact is not confirmed capture. */
  ...
  hold(0,180); assert(!output.valid && Promoted_LineRecovery_IsSearching());
  Promoted_line_tracking_reset(); assert(!BuzzerPhrase400_IsPlaying());
...
static void test_sampling_during_blocked_main(void)
{
  ...
  /* Exercise the real HAL tick override and GPIO acquisition, not injected
     controller input, while the main loop cannot call Promoted_line_tracking_compute. */
  LineSensorSample_Start();
  ...
  assert(telemetry.requested_cps[0]==LINE_SEARCH_TARGET_CPS);
  assert(!output.valid && Promoted_LineRecovery_IsSearching()); /* History does not fake capture. */
...
static void test_fast_exit_after_transverse(void)
{
  ...
  { uint32_t preserved=LineFaultLog_SearchCount(); Promoted_line_tracking_reset(); assert(LineFaultLog_SearchCount()==preserved); }
}
static void test_latest_outer_after_long_search(void)
{
  ...
  for(first=0;first<2;++first) for(queued=0;queued<2;++queued)
  {
    ...
    for(i=0;i<12;++i)
    {
      ...
      assert(!output.valid && Promoted_LineRecovery_IsSearching() && !brakes);
      hold(0,300);
    }
  }
...
static void test_gpio_snapshot_before_interrupt(void)
{
  ...
  {
    Promoted_LineTrackingReading snapshot;
    ...
    snapshot=Promoted_line_tracking_read();
    if(!at_restore) background_sample(interrupt_edge,1);
    assert(!test_irq_restore_hook);
    Promoted_line_tracking_compute(&snapshot,3000,&output);
...
static void test_overlapping_direction_and_interrupted_confirmation(void)
{
  ...
  for(right=0;right<2;++right)
  {
    ...
    if(!Promoted_LineRecovery_IsSearching()) ++false_capture;
...
static void test_direction_pattern_matrix(void)
{
  ...
  assert(Promoted_line_tracking_direction_evidence(0)==0);
  for(mask=0;mask<16;++mask)
  {
    Promoted_LineTrackingReading r={mask&1,(mask>>1)&1,(mask>>2)&1,(mask>>3)&1};
    assert(Promoted_line_tracking_direction_evidence(&r)==expected[mask]);
...
static void test_strong_exit_survives_inner_contact(void)
{
  ...
  for(right=0;right<2;++right)
  {
    ...
    assert(Promoted_LineRecovery_GetDirection()==(right?-1:1)); /* inner does not renew 200-ms exit age */

    reset(0,1); hold(right?2:8,30); hold(0,300);
    sample(edge,1,3000); sample(inner,1,3000); sample(right?2:8,1,3000); sample(0,1,3000);
    assert(Promoted_LineRecovery_GetDirection()==(right?-1:1)); /* newest outer wins */
    ...
    hold(0,120);
    assert(Promoted_LineRecovery_GetDirection()==(right?-1:1)); /* no stale pending exit after capture */
...
static void test_outer_escalation_boundaries(void)
{
  ...
  for(right=0;right<2;++right) for(wrap=0;wrap<2;++wrap)
  {
    ...
    Promoted_line_tracking_apply_command(&output,0);
    assert(telemetry.mode==DRIVE_BASE_STOPPED); /* zero cap still stops negative-target correction */
    Promoted_line_tracking_reset(); sample(outer,1,3000);
    assert(output.left_cps==(right?2200:0) && output.right_cps==(right?0:2200));
    ...
    {
      Promoted_LineTrackingReading frozen=Promoted_line_tracking_read();
      background_sample(outer,2);
      Promoted_line_tracking_compute(&frozen,3000,&output);
...
static void test_visible_forward_and_lost_spin(void)
{
  ...
  {
    reset(0,(uint8_t)smooth); Promoted_line_tracking_set_turn_gain_percent((uint16_t)gain);
    ...
    Promoted_line_tracking_reset(); assert(!BuzzerPhrase400_IsPlaying() && telemetry.requested_cps[0]==0);
    ++cases;
  }
...
{
  unsigned mask, i;
  Promoted_LineTrackingCommand baseline;
  reset(0,1); Promoted_line_tracking_set_straight_boost(1);
  ...
  Promoted_line_tracking_apply_command(&output,2600);
  assert(telemetry.requested_cps[0]==2600 && telemetry.requested_cps[2]==2600);
  Promoted_line_tracking_apply_command(&output,0);
  assert(telemetry.mode==DRIVE_BASE_STOPPED);
  Promoted_line_tracking_set_straight_boost(0); sample(5,10,3000);
  ...
  for(mask=0;mask<16;++mask)
  {
    ...
    reset(0,1); Promoted_line_tracking_set_straight_boost(1); hold(5,1000);
  ...
  Promoted_line_tracking_start_following(); hold(5,1000);
  assert(output.left_cps==2700); /* Mode 2 cannot inherit mode 1 acceleration. */
  Promoted_line_tracking_set_straight_boost(1); Promoted_line_tracking_rejoin_from_bypass(1);
  /* The mode-1 wrapper reapplies opt-in on each normal follow cycle. */
  Promoted_line_tracking_set_straight_boost(1); sample(5,1,3000);
  assert(output.left_cps==2200 && output.right_cps==2200);
  hold(5,1600); assert(output.left_cps==2850);
  sample(5,1,0); assert(output.valid && !output.left_cps && !output.right_cps);
  tick=UINT32_MAX-200; reset(0,1); Promoted_line_tracking_set_straight_boost(1);
  hold(5,1000); assert(output.left_cps==2850);
  Promoted_line_tracking_reset(); hold(5,1000); assert(output.left_cps==2700);
...
static void test_mode2_middle_guard(void)
{
  ...
  reset(0,1);
  Promoted_line_tracking_set_middle_guard(1U);
  sample(5,1,3000);
  assert(output.valid && output.action==Promoted_LINE_ACTION_FORWARD);
  assert(output.left_cps==1800 && output.right_cps==1800);

  sample(1,1,3000);
  assert(output.valid && output.action==Promoted_LINE_ACTION_LEFT_ADJUST);
  assert(output.left_cps==1412 && output.right_cps==1800);
  sample(4,1,3000);
  assert(output.valid && output.action==Promoted_LINE_ACTION_RIGHT_ADJUST);
  ...
  for(mirror=0;mirror<2;++mirror)
  {
    ...
    reset(0,1); Promoted_line_tracking_set_middle_guard(1U);
  ...
  reset(0,1); Promoted_line_tracking_set_middle_guard(1U);
  sample(15,1,3000);
  assert(output.valid && output.action==Promoted_LINE_ACTION_CROSSING);
  assert(output.left_cps==1800 && output.right_cps==1800);
...
int main(void)
{
  ...
  for(smooth=0;smooth<=1;++smooth) for(forward=0;forward<=1;++forward)
  {
    ...
    assert(!output.valid && telemetry.mode==DRIVE_BASE_SPEED && !brakes && Promoted_LineRecovery_IsSearching());
    ...
    hold(0,200); assert(!output.valid && Promoted_LineRecovery_IsSearching()); /* False contact resumes, never latches stop. */
    ...
    Promoted_line_tracking_reset(); assert(telemetry.mode==DRIVE_BASE_STOPPED && !BuzzerPhrase400_IsPlaying() && !buzzer);
    ...
    sample(5,10,3000); Promoted_line_tracking_reset();
    ...
    for(i=0;i<3;++i)
    {
      ...
      assert(output.valid && !output.left_cps && !BuzzerPhrase400_IsPlaying());
      assert(Promoted_LineRecovery_GetStopReason()==Promoted_LINE_REC_STOP_DRIVE_FAULT);
  ...
  Promoted_line_tracking_reset(); BuzzerPhrase400_Start(1); Promoted_line_tracking_reset();
