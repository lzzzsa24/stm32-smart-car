...
static Promoted_SignRouteCommand cmd;
static Promoted_SignRouteStatus status;
static Promoted_SimpleLineController line;
...
static void step(uint8_t mask, int32_t left_mm, int32_t right_mm)
{
  ...
  now += 10U;
  Promoted_SignRoute_UpdateEncoders((int32_t)lc,(int32_t)lc,(int32_t)rc,(int32_t)rc);
  Promoted_SignRoute_Step(mask,now,&cmd);
  Promoted_SignRoute_GetStatus(now,&status);
  Promoted_SimpleLine_StepRoute(&line,mask,&status,&cmd);
  ...
  if(mask!=0U && status.state!=Promoted_SIGN_ROUTE_FAULT)
...
static void init(uint32_t origin)
{
  ...
  Promoted_SimpleLine_Init(&line); Promoted_SimpleLine_Start(&line);
  Promoted_SignRoute_UpdateEncoders((int32_t)lc,(int32_t)lc,(int32_t)rc,(int32_t)rc);
}
...
static void enter_probe(void)
{
  ...
  assert(status.state==Promoted_SIGN_ROUTE_PROBE && !cmd.active);
  assert(motor_left>0 && motor_right>0);
}
static void leave_probe(uint8_t mask)
{
  unsigned i;
  for(i=0;i<20 && status.state==Promoted_SIGN_ROUTE_PROBE;++i) step(mask,2,2);
}
static void finish_select(int8_t side, Promoted_SignRouteState next)
{
  unsigned i;
  Promoted_SignRouteState selecting=status.state;
...
static void start_arc(int8_t side)
{
  ...
  assert(status.state==Promoted_SIGN_ROUTE_SELECTING);
  assert(side<0 ? cmd.left_pwm==0 : cmd.right_pwm==0);
  finish_select(side,Promoted_SIGN_ROUTE_ARC);
}
static void half_arc(int8_t side)
{
  ...
  for(i=0;i<200;++i)
  {
    ...
    assert(status.state==Promoted_SIGN_ROUTE_ARC && !cmd.active);
  }
...
static void test_exit(int8_t side, uint32_t origin)
{
  ...
  assert(status.state==Promoted_SIGN_ROUTE_ARC);
  for(i=0;i<4;++i) step(side<0?12U:3U,1,1);
  assert(status.state==Promoted_SIGN_ROUTE_EXIT_SELECT && cmd.active);
  assert(side<0 ? cmd.left_pwm==0 : cmd.right_pwm==0);
  finish_select(side,Promoted_SIGN_ROUTE_EXIT_CLEAR);
  for(i=0;i<50 && status.state!=Promoted_SIGN_ROUTE_LOCKED;++i) step(6U,2,2);
  assert(status.state==Promoted_SIGN_ROUTE_LOCKED && !cmd.active);
  /* No-target followed by silence is not sufficient for rearming. */
  observe(-1);
  for(i=0;i<180;++i) step(6U,0,0);
  assert(status.state==Promoted_SIGN_ROUTE_LOCKED);
  for(i=0;i<100;++i) { if(i%10==0) observe(-1); step(6U,0,0); }
  assert(status.state==Promoted_SIGN_ROUTE_IDLE);
...
static void test_crossbar_and_missing_sign(void)
{
  ...
  assert(status.state==Promoted_SIGN_ROUTE_ARMED); /* crossbar must not start turn */
  init(100); enter_probe(); leave_probe(6U);
  assert(status.state==Promoted_SIGN_ROUTE_IDLE); /* also crosses with no K210 */
  init(100); enter_probe(); leave_probe(0U);
  assert(status.state==Promoted_SIGN_ROUTE_WAIT_SIGN && !cmd.active);
  for(i=0;i<100;++i) { observe(3); step(1,0,0); }
  assert(status.state==Promoted_SIGN_ROUTE_WAIT_SIGN);
  confirm(-1); /* fresh arrows only */
  assert(status.state==Promoted_SIGN_ROUTE_ARMED && !cmd.active);
  Promoted_SignRoute_Reset(); step(6,0,0);
  assert(status.state==Promoted_SIGN_ROUTE_IDLE && !cmd.active);
...
static void test_bounds(void)
{
  ...
  assert(status.state==Promoted_SIGN_ROUTE_CANCELLED && status.fault==3U && !cmd.active);
  assert(motor_left>0 && motor_right>0); /* uncertain odometry cannot stop on-line */
  for(i=0;i<4;++i) step(3,0,0);
  assert(status.state==Promoted_SIGN_ROUTE_CANCELLED && !cmd.active); /* no invented exit */
  init(100); start_arc(-1);
  for(i=0;i<65;++i) step(0,0,0);
  assert(status.state==Promoted_SIGN_ROUTE_ARC && status.searching);
  for(i=0;i<10;++i) { observe(1); step(0,0,0); }
  assert(status.state==Promoted_SIGN_ROUTE_ARC && motor_left==-motor_right && motor_left!=0);
  for(i=0;i<4;++i) step(6,0,0);
  assert(status.state==Promoted_SIGN_ROUTE_ARC && motor_left>0 && motor_right>0);
  init(100); confirm(1); enter_probe(); leave_probe(0);
  for(i=0;i<260;++i) step(1,0,0);
  assert(status.state==Promoted_SIGN_ROUTE_CANCELLED && status.fault==2U && !cmd.active);
  for(i=0;i<4;++i) step(6,0,0);
  assert(status.state==Promoted_SIGN_ROUTE_CANCELLED && motor_left>0 && motor_right>0);
  init(100); confirm(1); enter_probe();
  for(i=0;i<190;++i) { step(15,0,0); assert(status.state!=Promoted_SIGN_ROUTE_FAULT); }
  init(100); start_arc(1); half_arc(1);
  for(i=0;i<4;++i) step(3,1,1);
  finish_select(1,Promoted_SIGN_ROUTE_EXIT_CLEAR);
  for(i=0;i<15;++i) step(0,1,1);
  assert(status.state==Promoted_SIGN_ROUTE_EXIT_CLEAR && !cmd.active);
  for(i=0;i<55;++i) step(0,0,0);
  assert(status.state==Promoted_SIGN_ROUTE_EXIT_CLEAR && status.searching);
...
static void test_live_line_priority(void)
{
  ...
  for(side=-1;side<=1;side+=2)
  {
    ...
    leave_probe(0);
    assert(status.state==Promoted_SIGN_ROUTE_SELECTING && !cmd.active);
    ...
    for(i=0;i<4;++i) step(6,0,0);
    assert(status.state==Promoted_SIGN_ROUTE_ARC); /* no minimum yaw gate */
  }
...
static void test_continuous_search(void)
{
  ...
  for(phase=0;phase<4;++phase)
  {
    ...
    for(i=0;i<12000;++i) /* two simulated minutes, no encoder progress */
    {
      ...
      assert(status.searching && status.state!=Promoted_SIGN_ROUTE_FAULT);
    }
    step(6,0,0); step(6,0,0);
    assert(!status.searching && motor_left>0 && motor_right>0);
    Promoted_SimpleLine_Stop(&line); Promoted_SignRoute_Reset(); step(0,0,0);
...
static void test_split_choice(int8_t side)
{
  ...
  for(repeat=0;repeat<3;++repeat)
  {
    ...
    assert(status.state==Promoted_SIGN_ROUTE_PROBE && status.direction==side);
    step(15,0,0);
  }
  for(i=0;i<5;++i) step(9,0,0);
  assert(status.state==Promoted_SIGN_ROUTE_SELECTING && status.direction==side && cmd.active);
...
static void test_exit_recovery(int8_t side)
{
  ...
  assert(status.state==Promoted_SIGN_ROUTE_ARC && !cmd.active); /* photo's all-black */
  for(i=0;i<10;++i) step(9,0,0);
  assert(status.state==Promoted_SIGN_ROUTE_ARC && !cmd.active); /* ambiguous both sides */
  ...
  assert(status.state==Promoted_SIGN_ROUTE_EXIT_SELECT); /* low-speed alignment gets >2.5 s */
  for(i=0;i<350;++i) { observe(side<0?0:1); step(15,0,0); }
  assert(status.state==Promoted_SIGN_ROUTE_CANCELLED && !cmd.active && status.direction==0);
  assert(motor_left>0 && motor_right>0);
  for(i=0;i<200;++i) { observe(side<0?0:1); step(side<0?12:3,0,0); }
  assert(status.state==Promoted_SIGN_ROUTE_CANCELLED && !cmd.active); /* same sign cannot retry */
  for(i=0;i<100;++i) { observe(-1); step(6,0,0); }
  assert(status.state==Promoted_SIGN_ROUTE_IDLE);
  confirm(-side); assert(status.state==Promoted_SIGN_ROUTE_ARMED && status.direction==-side);
