...
static Promoted_SignRouteCommand command;
...
static void step(uint8_t mask, int64_t yaw_mdeg, uint8_t yaw_valid, int32_t count_delta)
{
  ...
  Promoted_SignRoute_UpdateYaw(yaw_mdeg,yaw_valid);
  Promoted_SignRoute_Step(mask,now,&command);
...
static void begin_entry_turn(int side)
{
  ...
  Promoted_SignRoute_Reset(); Promoted_SignRoute_SetProfile(Promoted_SIGN_ROUTE_PROFILE_GYRO_TANGENT);
  Promoted_SignRoute_UpdateEncoders(0,0,0,0);
  Promoted_SignRoute_UpdateYaw(0,1);
  Promoted_SignRoute_UpdateObservationPause(1U,now);
  for(i=0;i<3;++i) { observe(side); step(6,0,1,0); }
  assert(status.state==Promoted_SIGN_ROUTE_ARMED && status.direction==side);
  ...
  Promoted_SignRoute_UpdateObservationPause(0U,now);
  step(6,0,1,0);
  assert(status.state==Promoted_SIGN_ROUTE_PROBE && command.active && !command.gentle_arc);
...
static void reach_arc(int side)
{
  ...
  assert(status.state==Promoted_SIGN_ROUTE_SELECTING && command.active);
  ...
  assert(status.state==Promoted_SIGN_ROUTE_ARC && status.entry_line_ready);
...
static void mode4_drawn_trajectory(int side)
{
  ...
  reach_arc(side);
  assert(status.profile==Promoted_SIGN_ROUTE_PROFILE_GYRO_TANGENT);

  /* Route never owns ARC motors, including during a white gap. The combined
     Promoted_SignLineFollow test verifies the forward fallback and live line output. */
  ...
  if(status.state!=Promoted_SIGN_ROUTE_EXIT_SELECT)
  ...
  assert(status.state==Promoted_SIGN_ROUTE_EXIT_SELECT && command.active && !command.gentle_arc);
  assert(side<0 ? (command.left_pwm==0 && command.right_pwm>0) :
                  (command.right_pwm==0 && command.left_pwm>0));
  ...
  assert(status.state==Promoted_SIGN_ROUTE_EXIT_SELECT && command.active);
  step(0,0,1,0);
  assert(status.state==Promoted_SIGN_ROUTE_EXIT_CLEAR && command.active);
  assert(command.left_pwm==command.right_pwm && command.left_pwm>0);
  step(0,0,1,600);
  for(i=0;i<4;++i) step(6,0,1,0);
  assert(status.state==Promoted_SIGN_ROUTE_LOCKED && !command.active);
}
...
static void entry_miss_returns_to_heading(int side)
{
  ...
  if(status.state!=Promoted_SIGN_ROUTE_ENTRY_RETURN)
  ...
  assert(status.state==Promoted_SIGN_ROUTE_ENTRY_RETURN && command.active);
  assert(side<0 ? (command.left_pwm>0 && command.right_pwm<0) :
                  (command.left_pwm<0 && command.right_pwm>0));
  assert(command.left_pwm==-command.right_pwm);

  step(0,0,1,0);
  assert(status.state==Promoted_SIGN_ROUTE_ENTRY_FALLBACK && command.active);
  assert(command.left_pwm==command.right_pwm && command.left_pwm>0);

  step(6,0,1,300); /* black on the very next sample must not be skipped */
  assert(status.state==Promoted_SIGN_ROUTE_ARC && status.direction==side &&
  ...
  assert(status.state==Promoted_SIGN_ROUTE_ARC && !command.active);
  ...
  assert(status.state==Promoted_SIGN_ROUTE_EXIT_SELECT && status.direction==side && command.active);
}
...
static void profile_isolation_and_invalid_gyro(void)
{
  ...
  Promoted_SignRoute_UpdateEncoders(0,0,0,0); Promoted_SignRoute_UpdateYaw(0,1);
  Promoted_SignRoute_UpdateObservationPause(1U,now);
  for(i=0;i<3;++i) { observe(-1); step(6,0,1,0); }
  Promoted_SignRoute_UpdateObservationPause(0U,now); step(6,0,1,0);
  assert(status.state==Promoted_SIGN_ROUTE_ARMED && !command.active);

  begin_entry_turn(1);
  step(6,0,0,0);
  assert(status.state==Promoted_SIGN_ROUTE_CANCELLED && !command.active && status.fault==6U);
