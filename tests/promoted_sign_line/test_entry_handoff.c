...
{
  now += 10U;
  Promoted_SignRoute_UpdateYaw(heading+angle,valid);
  Promoted_SimpleLine_UpdateYaw(&line,heading+angle,valid,1U);
  Promoted_SignRoute_Step(mask,now,&command);
  Promoted_SignRoute_GetStatus(now,&status);
  Promoted_SimpleLine_StepRoute(&line,mask,&status,&command);
  action=Promoted_SimpleLine_ResolveRouteOutput(&line,&command,paused,&left,&right);
}
...
static void init(int side, uint32_t origin)
{
  unsigned i;
  now=origin; seq=0;
  Promoted_SignRoute_Reset(); Promoted_SimpleLine_Init(&line); Promoted_SimpleLine_Start(&line);
  Promoted_SignRoute_UpdateEncoders(0,0,0,0);
  for(i=0;i<45;++i)
  {
    int32_t n=(int32_t)(i+1)*12;
    Promoted_SignRoute_UpdateEncoders(n,n,n,n); step(6,0,1,0);
  }
  assert(!status.road_reference_valid);
  for(i=0;i<3;++i) { if(side) observe(side); step(6,0,1,0); }
  for(i=0;i<3;++i) step(15,0,1,0);
  assert(status.state==Promoted_SIGN_ROUTE_PROBE);
}
...
static void expect_search(int side)
{
  ...
  assert(action==Promoted_SIMPLE_LINE_SEARCH);
}
...
static void check_handoff(int side, uint32_t origin)
{
  ...
  for(i=0;i<400;++i)
  {
    ...
    assert(-side*angle>=0 && -side*angle<=Promoted_SIMPLE_LINE_ENTRY_SEARCH_SECTOR_MDEG);
    assert(line.line_yaw_mdeg==heading);
  }
  ...
  assert(status.entry_line_ready && status.state==Promoted_SIGN_ROUTE_ARC);
  ...
  Promoted_SimpleLine_Stop(&line); step(selected,0,1,0);
  assert(command.active && left==0 && right==0); /* STOP wins even over active route */
  init(side,origin); step(0,0,0,0);
  assert(status.state==Promoted_SIGN_ROUTE_CANCELLED && left==0 && right==0);
  init(side,origin); now+=10001; step(opposite,0,1,0);
  assert(status.state==Promoted_SIGN_ROUTE_CANCELLED && status.direction==0);
...
static void check_early_arc(int side)
{
  ...
  for(angle=21;angle<=80;++angle)
  {
    ...
    assert(status.state==Promoted_SIGN_ROUTE_ARC && status.yaw_mdeg==0);
    assert(left>0 && right>0 && !command.active);
  }
