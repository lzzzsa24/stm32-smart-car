...
static void test(int side)
{
  ...
  assert(s.state==Promoted_SIGN_ROUTE_ARC && s.yaw_mdeg==0); /* stationary yaw must not lock apex */
  start(side);
  Promoted_SignRoute_UpdateEncoders(2000,2000,2000,2000);
  for(i=0;i<4;++i) step(side<0?12:3,entry+side*70000,1);
  assert(s.state==Promoted_SIGN_ROUTE_ARC); /* early edge cannot exit */
  step(6,entry-side*20000,1);
  assert(s.state==Promoted_SIGN_ROUTE_ARC && side*s.yaw_mdeg==-20000); /* locked apex cannot drift */
  ...
  for(j=0;j<sizeof(ambiguous);++j)
  {
    ...
    assert(s.state==Promoted_SIGN_ROUTE_ARC && !c.active); /* no exit from white, wide or both sides */
  }
  ...
  assert(s.state==Promoted_SIGN_ROUTE_EXIT_SELECT); /* no yaw: not completed */
  for(i=0;i<4;++i) step(0,0,1);
  assert(s.state==Promoted_SIGN_ROUTE_EXIT_CLEAR);
  assert(!c.active && c.left_pwm==0 && c.right_pwm==0);
  Promoted_SignRoute_UpdateEncoders(2500,2500,2500,2500);
  for(i=0;i<4;++i) step(6,0,1);
  assert(s.state==Promoted_SIGN_ROUTE_LOCKED);
  start(side);
  Promoted_SignRoute_UpdateEncoders(2000,2000,2000,2000);
  for(i=0;i<4;++i) step(6,entry+side*170000,1);
  ...
  for(i=0;i<4;++i) step(0,entry+side*170000,1);
  step(0,-side*35000,1);
  assert(s.state==Promoted_SIGN_ROUTE_EXIT_SELECT); /* outside both 25-degree alignment and 30-degree crossing bands */
  assert(side<0 ? c.left_pwm>0 && c.right_pwm==0 : c.right_pwm>0 && c.left_pwm==0);
  step(0,-side*5000,1);
  assert(s.state==Promoted_SIGN_ROUTE_EXIT_CLEAR && !c.active && c.left_pwm==0 && c.right_pwm==0);
  start(side);
  Promoted_SignRoute_UpdateEncoders(2000,2000,2000,2000);
  for(i=0;i<4;++i) step(6,entry+side*170000,1);
  for(i=0;i<4;++i) step(0,entry+side*170000,1);
  now+=6100; step(0,entry+side*170000,1);
  assert(s.state==Promoted_SIGN_ROUTE_CANCELLED && !c.active); /* stalled alignment remains bounded */
  ...
  assert(s.state==Promoted_SIGN_ROUTE_CANCELLED && !c.active); /* wrong half */
}
static void natural_exit(int side)
{
  ...
  assert(s.state==Promoted_SIGN_ROUTE_ARC && s.arc_peak_mdeg==160000);
  for(i=0;i<25;++i)
  { int32_t n=2000+(int32_t)(i+1)*16; Promoted_SignRoute_UpdateEncoders(n,n,n,n); step(6,0,1); }
  assert(s.state==Promoted_SIGN_ROUTE_LOCKED && !c.active && s.direction==0 && s.heading_error_mdeg==0);
  step(side<0?1:8,0,1); assert(!c.active); /* no forced straight over an outer contact */
  Promoted_SignRoute_UpdateEncoders(2500,2500,2500,2500);
  for(i=0;i<4;++i) step(6,0,1);
  assert(s.state==Promoted_SIGN_ROUTE_LOCKED && !c.active && s.direction==0);
  for(i=0;i<5;++i) step(0,entry+side*175000,1);
  assert(s.state==Promoted_SIGN_ROUTE_LOCKED && !c.active); /* no late exit-turn after natural rejoin */

  start(side);
  Promoted_SignRoute_UpdateEncoders(2000,2000,2000,2000);
  for(i=0;i<4;++i) step(6,entry+side*170000,1);
  step(0,entry+side*186000,1);
  assert(s.state==Promoted_SIGN_ROUTE_CANCELLED && !c.active); /* error worsens >15 deg: withdraw */

  start(side);
  Promoted_SignRoute_UpdateEncoders(2000,2000,2000,2000);
  for(i=0;i<4;++i) step(6,entry+side*170000,1);
  step(6,entry+side*186000,1);
  assert(s.state==Promoted_SIGN_ROUTE_CANCELLED && !c.active); /* same bound when ring line stays black */

  start(side);
  step(6,-side*120000,1); /* a larger estimated entry apex must not advance the exit */
  Promoted_SignRoute_UpdateEncoders(2000,2000,2000,2000);
  for(i=0;i<4;++i) step(6,side*50000,1);
  assert(s.state==Promoted_SIGN_ROUTE_ARC); /* phase yaw is 170, but signed road heading is only 50 */
