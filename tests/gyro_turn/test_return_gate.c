...
static void test_fixed_route(void)
{
  ...
  for(direction=-1;direction<=1;direction+=2)
  {
    ...
    fixed_step(&input); assert(travel_request==360 && travel_calls==1);
    ...
    fixed_step(&input); assert(travel_request==600 && travel_calls==2);
  ...
  puts("PASS: fixed 360/600/45 geometry, absolute-heading drift correction, all-phase STOP, obstacle fallback, generation/encoder fallback, capture gate and timer wrap");
}
