...
static unsigned send(int cls, unsigned dt)
{
  ...
  return SignHorn_Observe(&f,now);
}
int main(void)
{
  unsigned i;
  now=0xFFFFFF00U; f.sequence=0xFFFFFFFEU;
  SignHorn_Reset(); assert(!send(2,100)); assert(!send(2,100)); assert(send(2,100));
  for(i=0;i<100;++i) assert(!send(2,100));
  assert(!SignHorn_Observe(&f,now)); /* duplicate */
  ...
  SignHorn_Reset(); assert(!send(2,100)); assert(!send(0,100));
  assert(!send(2,100)); assert(!send(2,100)); assert(send(2,100));
  SignHorn_Reset(); assert(!send(2,100));
  f.sequence+=2; assert(!send(2,100)); assert(!send(2,100)); assert(send(2,100));
  SignHorn_Reset(); assert(!send(2,100));
  ++f.sequence; assert(!SignHorn_Observe(&f,now+1000));
  assert(!send(2,100)); assert(!send(2,100)); assert(send(2,100));
