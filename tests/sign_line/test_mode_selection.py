...
_Static_assert(APP_MODE_INTEGRATED==0 && APP_MODE_LINE_ONLY==1 &&
               APP_MODE_SIGN_LINE==2 && APP_MODE_RESERVED4==3 &&
               APP_MODE_VISION_LINE_V4==4 && APP_MODE_STOPPED==5,
               "Reserving key 4 must not renumber other mode telemetry");
int main(void)
{
  AppMode current;
  unsigned source, key;
  const uint8_t keys[]={IR_REMOTE_VIRTUAL_KEY1,IR_REMOTE_VIRTUAL_KEY2,
      IR_REMOTE_VIRTUAL_KEY3,IR_REMOTE_VIRTUAL_KEY5};
  const AppMode modes[]={APP_MODE_INTEGRATED,APP_MODE_LINE_ONLY,
      APP_MODE_SIGN_LINE,APP_MODE_VISION_LINE_V4};
  for(current=APP_MODE_INTEGRATED;current<=APP_MODE_STOPPED;++current)
  {
    for(source=0;source<2;++source)
    {
      /* Key 4 stops even if another source or a held physical key asks to run. */
      ...
      button=1; stops=0;
      assert(read_requested_mode(current)==APP_MODE_STOPPED && stops==2);
      for(key=0;key<4;++key)
      {
  ...
  puts("PASS: actual selector merges sign control into KEY3; KEY4/STOP preempt run requests; KEY1/2/5 numbers retained");
  return 0;
}
'''
