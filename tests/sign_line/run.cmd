...
if not "%errorlevel%"=="0" exit /b 1
set CL=
cl /nologo /W4 /WX /utf-8 /std:c11 /Itests\line_recovery\stubs /ICore\Inc tests\sign_line\test_low_speed_drive.c Core\Src\drive_base.c Core\Src\line_turn_load.c /Fomanual-build-sign-line-host-test\ /Femanual-build-sign-line-host-test\test_low_speed_drive.exe
if not "%errorlevel%"=="0" exit /b 1
manual-build-sign-line-host-test\test_low_speed_drive.exe
if not "%errorlevel%"=="0" exit /b 1
