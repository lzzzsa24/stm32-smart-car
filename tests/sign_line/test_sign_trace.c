...
int main(void)
{
  ...
  for(i=0;i<300;++i) { s.yaw_mdeg=(int32_t)i; SignTrace_Record(0xFFFFFF00U+i*20,12,&s,2200,0,3,6,0); }
  ...
  SignTrace_Record(0xFFFFFF00U+299*20+1,0,&s,0,0,0,0,0); /* immediate transition, no 20ms wait */
  assert(SignTrace_Get(255,&r) && r.fault==3);
  s.state=SIGN_ROUTE_IDLE;
  for(i=0;i<100;++i) SignTrace_Record(i*20,6,&s,0,0,0,1,0);
  ...
  SignTrace_Record(10000,6,&s,1412,1412,1,1,0);
  assert(SignTrace_Get(255,&r) && r.state==SIGN_ROUTE_ARMED && r.left_cps==1412);
  SignTrace_Record(10001,0,&s,1870,1412,2,6,0);
  assert(SignTrace_Get(255,&r) && r.mask==0 && r.control_owner==6 &&
