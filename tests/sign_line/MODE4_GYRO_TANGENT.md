...
1. `ARM/OBSERVE`：三帧确认左右方向并完成两秒观察停车。只有“停车曾经开始、
   ...
   最外侧传感器后才转向。
2. `ENTRY TURN`：以停车结束时的航向为原点，按识别方向进行单侧前进枢轴转向，
   陀螺仪到 45°后停止转向。该动作不是原地反向旋转。
...
- 完整 `gyro_turn` 回归通过，确认共享 MPU 服务、模式 1 绕障角度和模式 4
  新绑定可以共同编译运行。
- 正式 ARM 构建通过：text/data/bss = 111272/64/18344，BIN 111340 字节；
  BIN SHA256 `D30047782F100D71A87E98C09900B0E123DA3C2CB1290B16968E2B47600CF3BE`，
  HEX SHA256 `8B89333C7D7F6C47A84F9D5B6B434BCC64E770CCED6A44FA6F1385BAC0E0A1E1`。
