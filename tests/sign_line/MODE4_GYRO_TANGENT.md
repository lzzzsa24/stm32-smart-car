...
3. `ENTRY LINE`：两侧同速直行，先确认已经离开入口黑线，再继续沿图中斜线行驶；
   ...
   原地转回停车前航向，再进入 `FALLBACK LINE` 两轮同速直行；重新见线后交还
   普通循线。回正失败或直行 450 mm/3.5 s 仍未见线时撤销路线但不停车。
...
45°转向、斜线直行、80 mm 未见弧回正、回正后直行、90°圆弧、出口回正、
两侧持续正向圆弧和陀螺仪失效撤销、
...
- 完整 `gyro_turn` 回归通过，确认共享 MPU 服务、模式 1 绕障角度和模式 4
  新绑定可以共同编译运行。
- 正式 ARM 构建通过：text/data/bss = 112056/64/18344，BIN 112124 字节；
  BIN SHA256 `762FBB0C76703D71F9600B5A8B94EA23068A463193A4ED28F7E0BFCBE45A0320`，
  HEX SHA256 `FA32CCC376BCE053C71DB850F79714E961009594E77D48EFB781013D22ED96FD`。
