...
static LineObstacleBypassInput setup(int direction)
{
  ...
  LineObstacleBypassConfig cfg; LineObstacleBypass_GetDefaultConfig(&cfg); cfg.adaptive_return_max_cps=2100U;
  LineObstacleBypass_Init(&cfg); assert(LineObstacleBypass_Start((int8_t)direction));
...
static LineObstacleBypassInput fixed_setup(int direction)
{
  ...
  LineObstacleBypass_GetDefaultConfig(&config); config.adaptive_return_max_cps=2100U; config.fixed_route_direction=(int8_t)direction;
...
static void test_fixed_route(void)
{
  ...
  for(direction=-1;direction<=1;direction+=2)
  {
    ...
    fixed_step(&input); assert(travel_request==300 && travel_calls==2);
  ...
  puts("PASS: fixed 250/300/45 geometry, absolute-heading drift correction, all-phase STOP, obstacle fallback, generation/encoder fallback, capture gate and timer wrap");
}
