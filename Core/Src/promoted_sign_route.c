...
#include "promoted_sign_route_config.h"
...
typedef struct
{
  ...
  Promoted_SignRouteProfile profile;
  int32_t arc_peak_mdeg;
  int32_t exit_best_error_mdeg;
  uint8_t road_reference_valid, motion_forward;
  uint8_t motion_valid;
  uint8_t exit_reason;
  uint32_t motion_ms;
  int64_t step_left, step_right;
  int64_t sweep_min_yaw, sweep_max_yaw;
  int32_t arc_sweep_mdeg;
} Promoted_SignRouteContext;

static Promoted_SignRouteContext route;
static uint8_t mode3_exit_angle_deg = (uint8_t)(SIGN_EXIT_HEADING_TRIGGER_MDEG / 1000L);

uint8_t Promoted_SignRoute_GetExitAngleDegrees(void) { return mode3_exit_angle_deg; }
void Promoted_SignRoute_AdjustExitAngle(int8_t direction)
{
...
static void enter_phase(Promoted_SignRouteState state, uint32_t now);
...
static void observe_forward_motion(uint32_t now)
{
  /* Main-loop samples can be 1 ms apart: no encoder edge in one sample is
     normal quantization, not proof that the car stopped. Use a short window. */
  if (!route.motion_valid || now-route.motion_ms>=10U ||
      route.left_counts<route.step_left || route.right_counts<route.step_right)
  {
    route.motion_forward=route.motion_valid && now-route.motion_ms<=SIGN_SAMPLE_MAX_GAP_MS &&
        route.left_counts>route.step_left && route.right_counts>route.step_right;
    route.motion_valid=1U; route.motion_ms=now;
    route.step_left=route.left_counts; route.step_right=route.right_counts;
  }
}
...
static void try_confirm(uint32_t now)
{
  ...
  if ((route.state != Promoted_SIGN_ROUTE_IDLE && route.state != Promoted_SIGN_ROUTE_PROBE &&
       route.state != Promoted_SIGN_ROUTE_WAIT_SIGN) || route.direction != 0 || route.count < 3U)
  {
    return;
  }
  ...
  for (i = 0U; i < route.count; ++i)
  {
    ...
    {
      ++votes;
      if (route.score[i] >= Promoted_SIGN_OBSERVATION_MODE3_SCORE_MINIMUM)
        observation_vote = 1U;
    }
  }
  ...
  if (votes >= 3U && (route.profile != Promoted_SIGN_ROUTE_PROFILE_STANDARD || observation_vote))
  {
    route.direction = candidate == 0 ? -1 : 1;
    if (route.state == Promoted_SIGN_ROUTE_IDLE) route.state = Promoted_SIGN_ROUTE_ARMED;
    route.armed_ms = now;
    route.junction_active = 0U;
  }
}

void Promoted_SignRoute_Init(void)
{
  Promoted_SignRoute_Reset();
}

void Promoted_SignRoute_Reset(void)
{
  Promoted_SignRouteProfile profile = route.profile;
  memset(&route, 0, sizeof(route));
  route.profile = profile;
  route.last_class = -1;
}

void Promoted_SignRoute_SetProfile(Promoted_SignRouteProfile profile)
{
  route.profile = profile == Promoted_SIGN_ROUTE_PROFILE_GYRO_TANGENT ?
      Promoted_SIGN_ROUTE_PROFILE_GYRO_TANGENT : Promoted_SIGN_ROUTE_PROFILE_STANDARD;
}

void Promoted_SignRoute_MarkObservationSearch(void)
{
  if (route.profile==Promoted_SIGN_ROUTE_PROFILE_STANDARD) route.observation_search_seen=1U;
}

void Promoted_SignRoute_UpdateObservationPause(uint8_t paused, uint32_t now)
{
  paused = paused ? 1U : 0U;
  ...
  if (!paused && route.observation_pause_active &&
      route.profile == Promoted_SIGN_ROUTE_PROFILE_STANDARD)
  {
  ...
  if (route.profile != Promoted_SIGN_ROUTE_PROFILE_GYRO_TANGENT)
  {
    /* The user-selected highest-priority reference is the final stop pose.
       Neither later line following nor entry/search yaw may replace it. */
    ...
    if (!paused && route.observation_pause_active && route.imu_valid &&
        (route.state==Promoted_SIGN_ROUTE_IDLE || route.state==Promoted_SIGN_ROUTE_ARMED ||
         route.state==Promoted_SIGN_ROUTE_PROBE || route.state==Promoted_SIGN_ROUTE_WAIT_SIGN ||
         route.state==Promoted_SIGN_ROUTE_SELECTING))
    {
      route.approach_yaw=route.imu_yaw;
      route.approach_from_pause=route.road_reference_valid=1U;
  ...
  if (paused)
  {
    route.observation_pause_seen = 1U;
           ...
           route.state == Promoted_SIGN_ROUTE_ARMED && route.direction != 0)
  {
    ...
    enter_phase(Promoted_SIGN_ROUTE_PROBE, now);
    route.observation_pause_seen = 0U;
  }
  route.observation_pause_active = paused;
}

void Promoted_SignRoute_ObserveDetection(const VisionDetection *detection)
{
  int8_t candidate;
  uint32_t now;
...
void Promoted_SignRoute_UpdateEncoders(int32_t m1, int32_t m2, int32_t m3, int32_t m4)
{
...
void Promoted_SignRoute_UpdateYaw(int64_t yaw_mdeg, uint8_t valid)
{
  route.imu_yaw = yaw_mdeg;
  route.imu_valid = valid;
  if (!valid && route.profile==Promoted_SIGN_ROUTE_PROFILE_STANDARD)
    route.road_reference_valid=0U;
}

static void enter_phase(Promoted_SignRouteState state, uint32_t now)
{
  Promoted_SignRouteState previous_state=route.state;
  if (state == Promoted_SIGN_ROUTE_PROBE)
  {
    ...
    if (route.profile != Promoted_SIGN_ROUTE_PROFILE_STANDARD || !route.road_reference_valid)
    {
      ...
      if (route.profile==Promoted_SIGN_ROUTE_PROFILE_STANDARD) route.road_reference_valid=route.imu_valid;
    }
  }
  route.state = state;
  route.phase_ms = now;
  ...
  if (state == Promoted_SIGN_ROUTE_PROBE || state == Promoted_SIGN_ROUTE_SELECTING)
...
static void update_geometry(void)
{
...
#if Promoted_SIGN_ROUTE_REQUIRE_IMU
  {
    ...
    {
      if ((route.state == Promoted_SIGN_ROUTE_PROBE || route.state == Promoted_SIGN_ROUTE_SELECTING) &&
          -route.direction * (route.imu_yaw-route.entry_extreme_yaw) > 0)
        route.entry_extreme_yaw=route.imu_yaw;
      if (route.state == Promoted_SIGN_ROUTE_ARC && !route.arc_origin_locked)
      {
        ...
        if (route.direction * (route.imu_yaw-route.imu_origin) < 0 &&
          ...
          route.imu_origin=route.imu_yaw;
        if ((route.profile == Promoted_SIGN_ROUTE_PROFILE_GYRO_TANGENT || is_track_line(route.last_line_mask)) &&
...
static void observe_arc_sweep(uint8_t mask)
{
  ...
  if (!is_track_line(mask) || !route.motion_forward) return;
  if (yaw<route.sweep_min_yaw)
  {
    route.sweep_min_yaw=yaw;
    if (!route.arc_sweep_mdeg) route.sweep_max_yaw=yaw;
  }
  if (yaw>route.sweep_max_yaw) route.sweep_max_yaw=yaw;
  route.arc_sweep_mdeg=(int32_t)(route.sweep_max_yaw-route.sweep_min_yaw);
}

#endif

static void capture_arc(uint32_t now, Promoted_SignRouteCommand *command)
{
  enter_phase(Promoted_SIGN_ROUTE_ARC, now);
...
static void observe_entry_line(uint8_t mask, uint32_t now)
{
  ...
  int32_t minimum_yaw = route.profile == Promoted_SIGN_ROUTE_PROFILE_GYRO_TANGENT ?
...
static void tangent_command(Promoted_SignRouteCommand *command, int8_t direction)
{
...
static void straight_command(Promoted_SignRouteCommand *command)
{
  command->active = 1U;
  command->gentle_arc = 0U;
  command->left_pwm = Promoted_SIGN_ROUTE_PWM;
  command->right_pwm = Promoted_SIGN_ROUTE_PWM;
}

static void pivot_command(Promoted_SignRouteCommand *command, int8_t direction)
{
  if (!direction) return;
  command->active = 1U;
  command->gentle_arc = 0U;
  command->left_pwm = direction < 0 ? 0 : Promoted_SIGN_ROUTE_PWM;
  command->right_pwm = direction < 0 ? Promoted_SIGN_ROUTE_PWM : 0;
}

static void spin_command(Promoted_SignRouteCommand *command, int8_t direction)
{
  if (!direction) return;
  command->active = 1U;
  command->gentle_arc = 0U;
  command->left_pwm = direction < 0 ? -Promoted_SIGN_ROUTE_PWM : Promoted_SIGN_ROUTE_PWM;
...
static uint8_t gyro_tangent_step(uint8_t line_mask, uint32_t now,
                                 Promoted_SignRouteCommand *command)
{
  int32_t turn_yaw;
  int64_t heading_error;

  if (route.profile != Promoted_SIGN_ROUTE_PROFILE_GYRO_TANGENT) return 0U;
  if (route.state == Promoted_SIGN_ROUTE_IDLE || route.state == Promoted_SIGN_ROUTE_ARMED ||
      route.state == Promoted_SIGN_ROUTE_WAIT_SIGN)
    return 1U;

  if (route.state == Promoted_SIGN_ROUTE_PROBE)
  {
    ...
    {
      enter_phase(Promoted_SIGN_ROUTE_SELECTING, now);
      command->just_finished = 1U;
  ...
  if (route.state == Promoted_SIGN_ROUTE_SELECTING)
  {
    ...
    if (route.travel_mm >= SIGN_GYRO_TANGENT_ENTRY_SEARCH_MM)
    {
      ...
      enter_phase(Promoted_SIGN_ROUTE_ENTRY_RETURN, now);
  ...
  if (route.state == Promoted_SIGN_ROUTE_ENTRY_RETURN)
  {
    turn_yaw = route.direction * route.yaw_mdeg;
    heading_error = route.imu_yaw - route.approach_yaw;
    if (now - route.phase_ms > SIGN_GYRO_TANGENT_TURN_TIMEOUT_MS ||
    ...
    {
      enter_phase(Promoted_SIGN_ROUTE_ENTRY_FALLBACK, now);
  ...
  if (route.state == Promoted_SIGN_ROUTE_ENTRY_FALLBACK)
  {
  ...
  if (route.state == Promoted_SIGN_ROUTE_ARC)
  {
    ...
    /* ARC motor ownership remains with Promoted_SignLineFollow for both visible line
  ...
  if (route.state == Promoted_SIGN_ROUTE_EXIT_CLEAR)
  {
    if (line_mask == 0U) route.departed = 1U;
...
static void select_command(Promoted_SignRouteCommand *command, uint8_t mask)
{
  uint8_t selected_edge = route.direction < 0 ? 8U : 1U;
#if Promoted_SIGN_ROUTE_REQUIRE_IMU
  if (route.direction && route.imu_valid && route.state == Promoted_SIGN_ROUTE_EXIT_SELECT)
  {
    ...
    /* Exit state starts from heading alone. Keep the existing wide-line
       motor handoff; sensor contact still governs exit completion. */
    if (is_junction(mask)) return;
    steer = heading_error() > 0 ? 1 : -1;
    /* Existing KEY2 forward pivot; never request counter-rotation here. */
    command->active = 1U;
    route.departed = 1U;
    command->left_pwm = steer < 0 ? 0 : Promoted_SIGN_ROUTE_PWM;
    command->right_pwm = steer < 0 ? Promoted_SIGN_ROUTE_PWM : 0;
    return;
  }
#endif
  if ((route.state == Promoted_SIGN_ROUTE_PROBE || route.state == Promoted_SIGN_ROUTE_SELECTING) &&
  ...
  command->left_pwm = route.direction < 0 ? 0 : Promoted_SIGN_ROUTE_PWM;
  command->right_pwm = route.direction < 0 ? Promoted_SIGN_ROUTE_PWM : 0;
}

void Promoted_SignRoute_Step(uint8_t line_mask, uint32_t now, Promoted_SignRouteCommand *command)
{
  ...
  if (route.state == Promoted_SIGN_ROUTE_ARMED &&
      route.direction != 0 &&
       ...
       now - route.last_frame_ms > SIGN_ONLINE_MAX_AGE_MS))
  {
    ...
    route.road_reference_valid=0U;
    clear_window();
    if (route.state == Promoted_SIGN_ROUTE_ARMED) route.state = Promoted_SIGN_ROUTE_IDLE;
  }
  if (route.state == Promoted_SIGN_ROUTE_LOCKED || route.state == Promoted_SIGN_ROUTE_CANCELLED)
  {
    if (now - route.finished_ms >= SIGN_REARM_COOLDOWN_MS &&
        ...
        route.last_class == -1)
    {
      route.state = Promoted_SIGN_ROUTE_IDLE;
      route.direction = 0;
      route.fault = 0U;
      route.junction_active = 0U;
      clear_window();
  ...
  if (route.state == Promoted_SIGN_ROUTE_PROBE)
  {
        ...
        now - route.phase_ms > SIGN_PROBE_TIMEOUT_MS)
    {
      ...
      route.state = route.direction ? Promoted_SIGN_ROUTE_ARMED : Promoted_SIGN_ROUTE_IDLE;
        ...
        (!center || now - route.capture_since_ms >= SIGN_PROBE_CENTER_CLEAR_MS))
    {
      ...
      {
        enter_phase(Promoted_SIGN_ROUTE_WAIT_SIGN, now);
        clear_window();
      }
    }
    return;
  ...
  if (route.state == Promoted_SIGN_ROUTE_WAIT_SIGN)
  {
  ...
  if (route.state == Promoted_SIGN_ROUTE_SELECTING || route.state == Promoted_SIGN_ROUTE_EXIT_SELECT)
  {
    ...
    if (now - route.phase_ms > timeout ||
        (route.state == Promoted_SIGN_ROUTE_EXIT_SELECT && route.profile == Promoted_SIGN_ROUTE_PROFILE_STANDARD ?
    ...
    if (route.state == Promoted_SIGN_ROUTE_SELECTING && route.entry_line_ready)
    {
      capture_arc(now, command);
      return;
    }
    ...
    if (route.state == Promoted_SIGN_ROUTE_EXIT_SELECT)
    {
      ...
      if (turn_yaw >= SIGN_SELECT_CAPTURE_MIN_MDEG && aligned)
      {
        ...
        command->active=1U;
        command->gentle_arc=0U;
        command->left_pwm=command->right_pwm=Promoted_SIGN_ROUTE_PWM;
        return;
      }
    }
#endif
  ...
  if (route.state == Promoted_SIGN_ROUTE_ARC)
  {
    ...
    if (route.road_reference_valid &&
        (road_heading < -SIGN_ENTRY_MAX_MDEG || road_heading > SIGN_EXIT_HEADING_MAX_MDEG))
    {
      cancel_route(3U, now, command);
      return;
    }
    ...
    else if (route.profile == Promoted_SIGN_ROUTE_PROFILE_GYRO_TANGENT)
    {
  ...
  if (route.state == Promoted_SIGN_ROUTE_EXIT_CLEAR)
  {
    uint32_t exit_timeout = route.profile == Promoted_SIGN_ROUTE_PROFILE_GYRO_TANGENT ?
        SIGN_GYRO_TANGENT_EXIT_TIMEOUT_MS : SIGN_EXIT_CLEAR_TIMEOUT_MS;
    int32_t exit_max_mm = route.profile == Promoted_SIGN_ROUTE_PROFILE_GYRO_TANGENT ?
        SIGN_GYRO_TANGENT_EXIT_MAX_MM : SIGN_EXIT_STRAIGHT_MAX_MM;
    int32_t exit_min_mm = route.profile == Promoted_SIGN_ROUTE_PROFILE_GYRO_TANGENT ?
    ...
    {
      if (route.profile == Promoted_SIGN_ROUTE_PROFILE_STANDARD) complete_route(now,command);
      else
      {
        route.state = Promoted_SIGN_ROUTE_LOCKED;
        route.finished_ms = now;
...
void Promoted_SignRoute_GetStatus(uint32_t now, Promoted_SignRouteStatus *status)
{
  ...
  status->road_reference_valid=route.road_reference_valid;
  status->exit_reason=route.exit_reason;
  status->arc_sweep_mdeg=route.arc_sweep_mdeg;
}
