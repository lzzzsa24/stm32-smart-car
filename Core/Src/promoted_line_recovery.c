...
static Promoted_LineRecoveryStopReason stop_reason;
...
uint8_t Promoted_LineRecovery_IsSearching(void) { return phase == REC_SEARCH; }
Promoted_LineRecoveryStopReason Promoted_LineRecovery_GetStopReason(void) { return stop_reason; }
int8_t Promoted_LineRecovery_GetDirection(void) { return side; }
...
void Promoted_LineRecovery_Reset(void)
{
  ...
  phase = REC_IDLE;
  stop_reason = Promoted_LINE_REC_STOP_NONE;
  center_candidate = 0U;
  side = exit_side = 0;
  exit_edge_seen = 0U;
  uncertain_search = 0U;
...
void Promoted_LineRecovery_Begin(int8_t preferred_side, uint32_t now)
{
  ...
  phase = REC_SEARCH;
  center_last_ms = now;
  if (DriveBase_GetFaultMask()) Promoted_LineRecovery_Stop(Promoted_LINE_REC_STOP_DRIVE_FAULT);

}
void Promoted_LineRecovery_BeginAmbiguous(int8_t initial_side, uint32_t now)
{
  DriveBaseTelemetry telemetry;
  Promoted_LineRecovery_Begin(initial_side, now);
...
void Promoted_LineRecovery_ObserveDirection(const Promoted_LineTrackingReading *r, uint32_t now)
{
...
static Promoted_LineRecoveryResult step_recovery(const Promoted_LineTrackingReading *r,
                                     ...
                                     uint8_t immediate_capture)
{
  ...
  DriveBase_Task(now);
  if (DriveBase_GetFaultMask()) Promoted_LineRecovery_Stop(Promoted_LINE_REC_STOP_DRIVE_FAULT);
  if (phase == REC_FAULT) return Promoted_LINE_RECOVERY_FAILED;
  if (phase == REC_CAPTURED) return Promoted_LINE_RECOVERY_CAPTURED;
  Promoted_LineRecovery_ObserveDirection(r, now);
  DriveBase_GetTelemetry(&telemetry);
  ...
  if (telemetry.mode == DRIVE_BASE_BRAKING) return Promoted_LINE_RECOVERY_BUSY;
  if (phase == REC_SEARCH)
  {
    Promoted_LineRecovery_ObserveDirection(r, now);
    if (uncertain_search &&
    ...
    else
    {
          ...
          (immediate_capture == 2U ? now != center_last_ms : now - center_since >= SENSOR_CONFIRM_MS)))
      {
        phase = REC_CAPTURED;
        exit_edge_seen = 0U;
        return Promoted_LINE_RECOVERY_CAPTURED;
      }
  ...
  return Promoted_LINE_RECOVERY_BUSY;
}

Promoted_LineRecoveryResult Promoted_LineRecovery_Step(const Promoted_LineTrackingReading *r,
                                     Promoted_LineTrackingCommand *command, uint32_t now)
{ return step_recovery(r, command, now, 0U); }

Promoted_LineRecoveryResult Promoted_LineRecovery_StepImmediate(const Promoted_LineTrackingReading *r,
                                             Promoted_LineTrackingCommand *command, uint32_t now)
{ return step_recovery(r, command, now, 1U); }

Promoted_LineRecoveryResult Promoted_LineRecovery_StepRolling(const Promoted_LineTrackingReading *r,
                                           Promoted_LineTrackingCommand *command, uint32_t now)
{ return step_recovery(r, command, now, 2U); }

Promoted_LineRecoveryResult Promoted_LineRecovery_StepCentering(const Promoted_LineTrackingReading *r,
                                             Promoted_LineTrackingCommand *command, uint32_t now)
