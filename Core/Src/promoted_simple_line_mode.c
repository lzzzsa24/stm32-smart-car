...
#define Promoted_SIMPLE_LINE_FILTER_SAMPLES 2U
#define Promoted_SIMPLE_LINE_STRAIGHT_PWM 2400
#define Promoted_SIMPLE_LINE_SLOW_PWM     2200
#define Promoted_SIMPLE_LINE_OUTER_PWM    2600
#define Promoted_SIMPLE_LINE_TURN_PWM     2700

_Static_assert(Promoted_SIMPLE_LINE_ENTRY_SEARCH_SECTOR_MDEG < SIGN_ENTRY_MAX_MDEG,
               "Entry search must reverse before the route yaw cancellation bound");

static void set_output(Promoted_SimpleLineController *controller,
...
static void set_turn(Promoted_SimpleLineController *controller, int8_t direction)
{
  int16_t left = direction < 0 ? -Promoted_SIMPLE_LINE_TURN_PWM :
                                 Promoted_SIMPLE_LINE_TURN_PWM;
  set_output(controller, Promoted_SIMPLE_LINE_TURN, left, (int16_t)-left);
}
...
void Promoted_SimpleLine_Init(Promoted_SimpleLineController *controller)
{
  ...
  controller->mode = Promoted_SIMPLE_LINE_STOP;
}

void Promoted_SimpleLine_Start(Promoted_SimpleLineController *controller)
{
  if (controller == NULL || controller->mode != Promoted_SIMPLE_LINE_STOP)
  {
    return;
  }
  Promoted_SimpleLine_Init(controller);
  controller->mode = Promoted_SIMPLE_LINE_TRACK;
}
...
void Promoted_SimpleLine_Stop(Promoted_SimpleLineController *controller)
{
  if (controller != NULL)
  {
    set_output(controller, Promoted_SIMPLE_LINE_STOP, 0, 0);
  }
}
...
void Promoted_SimpleLine_StepArc(Promoted_SimpleLineController *controller, uint8_t raw_mask)
{
  ...
  if (controller == NULL || controller->mode == Promoted_SIMPLE_LINE_STOP || mask == 0U)
    return;
  if (mask == 8U || mask == 12U || mask == 4U)
  {
    controller->last_direction = -1;
    set_output(controller, Promoted_SIMPLE_LINE_TRACK, Promoted_SIMPLE_LINE_SLOW_PWM, Promoted_SIMPLE_LINE_OUTER_PWM);
  }
  else if (mask == 1U || mask == 3U || mask == 2U)
  {
    controller->last_direction = 1;
    set_output(controller, Promoted_SIMPLE_LINE_TRACK, Promoted_SIMPLE_LINE_OUTER_PWM, Promoted_SIMPLE_LINE_SLOW_PWM);
  }
  else
    set_output(controller, Promoted_SIMPLE_LINE_TRACK, Promoted_SIMPLE_LINE_SLOW_PWM, Promoted_SIMPLE_LINE_SLOW_PWM);
}
...
void Promoted_SimpleLine_StepSlow(Promoted_SimpleLineController *controller, uint8_t raw_mask)
{
  ...
  if (mask == Promoted_SIMPLE_LINE_LEFT_INNER || mask == Promoted_SIMPLE_LINE_RIGHT_INNER)
  {
    int8_t direction = mask == Promoted_SIMPLE_LINE_LEFT_INNER ? -1 : 1;
    controller->last_direction = direction;
    set_output(controller, Promoted_SIMPLE_LINE_TRACK,
  ...
  else if (mask == 8U || mask == 12U || mask == 1U || mask == 3U)
  {
    ...
    set_output(controller, mask == 6U ? Promoted_SIMPLE_LINE_TRACK : Promoted_SIMPLE_LINE_WIDE, 2300, 2300);
}
                          ...
                          const Promoted_SignRouteStatus *route, const Promoted_SignRouteCommand *command)
{
  ...
  uint8_t entry_guard;
  int64_t lower, upper;
  if (!controller || !route || !command || controller->mode == Promoted_SIMPLE_LINE_STOP) return;
  entry_guard = controller->yaw_configured && route->yaw_valid && route->direction &&
      !route->entry_line_ready &&
      (route->state == Promoted_SIGN_ROUTE_PROBE || route->state == Promoted_SIGN_ROUTE_SELECTING);
  ...
  if (route->state == Promoted_SIGN_ROUTE_PROBE && route->direction && !controller->route_hint)
  {
    Promoted_SimpleLine_SetDirection(controller, route->direction);
    controller->route_hint = route->direction;
  }
  else if (route->state != Promoted_SIGN_ROUTE_PROBE && route->state != Promoted_SIGN_ROUTE_ARC &&
  ...
  Promoted_SimpleLine_StepSlow(controller, tracking_mask);
    ...
    if (controller->yaw_valid)
    {
      ...
      if (route->state == Promoted_SIGN_ROUTE_ARC)
      {
        ...
        if (controller->curve_yaw_valid && tracking_mask == 6U &&
            (curve_delta >= Promoted_SIMPLE_LINE_ARC_TREND_MDEG ||
             curve_delta <= -Promoted_SIMPLE_LINE_ARC_TREND_MDEG))
        {
  ...
  if (!controller->yaw_valid)
  {
    ...
    set_output(controller,Promoted_SIMPLE_LINE_SEARCH,0,0);
    return;
  }
  ...
  lower = -Promoted_SIMPLE_LINE_SEARCH_SECTOR_MDEG;
  upper = Promoted_SIMPLE_LINE_SEARCH_SECTOR_MDEG;
  if (entry_guard)
  {
    ...
    { lower=phase_origin; upper=lower+Promoted_SIMPLE_LINE_ENTRY_SEARCH_SECTOR_MDEG; }
    else
    { upper=phase_origin; lower=upper-Promoted_SIMPLE_LINE_ENTRY_SEARCH_SECTOR_MDEG; }
  }
  ...
  controller->mode=Promoted_SIMPLE_LINE_SEARCH;
}
...
{
  *left = *right = 0;
  if (controller->mode == Promoted_SIMPLE_LINE_STOP) return Promoted_SIMPLE_LINE_STOP;
  if (paused) return 6U;
  if (command->active)
  {
...
void Promoted_SimpleLine_Step(Promoted_SimpleLineController *controller, uint8_t raw_mask)
{
  ...
  else if (controller->sample_count < Promoted_SIMPLE_LINE_FILTER_SAMPLES)
  {
    ++controller->sample_count;
  }
  if (controller->sample_count >= Promoted_SIMPLE_LINE_FILTER_SAMPLES)
  {
    controller->filtered_mask = controller->candidate_mask;
  }
  if (controller->mode == Promoted_SIMPLE_LINE_STOP)
  {
    return;
  }
  ...
  if (value == (Promoted_SIMPLE_LINE_LEFT_INNER | Promoted_SIMPLE_LINE_RIGHT_INNER))
  {
    set_output(controller, Promoted_SIMPLE_LINE_TRACK,
               Promoted_SIMPLE_LINE_STRAIGHT_PWM, Promoted_SIMPLE_LINE_STRAIGHT_PWM);
  }
  else if (value == Promoted_SIMPLE_LINE_LEFT_INNER ||
           value == Promoted_SIMPLE_LINE_RIGHT_INNER)
  {
    direction = value == Promoted_SIMPLE_LINE_LEFT_INNER ? -1 : 1;
    controller->last_direction = direction;
    set_output(controller, Promoted_SIMPLE_LINE_TRACK,
               direction < 0 ? Promoted_SIMPLE_LINE_SLOW_PWM : Promoted_SIMPLE_LINE_OUTER_PWM,
               direction < 0 ? Promoted_SIMPLE_LINE_OUTER_PWM : Promoted_SIMPLE_LINE_SLOW_PWM);
  }
  else if (value == Promoted_SIMPLE_LINE_LEFT_OUTER ||
           value == (Promoted_SIMPLE_LINE_LEFT_OUTER | Promoted_SIMPLE_LINE_LEFT_INNER) ||
           value == Promoted_SIMPLE_LINE_RIGHT_OUTER ||
           value == (Promoted_SIMPLE_LINE_RIGHT_OUTER | Promoted_SIMPLE_LINE_RIGHT_INNER))
  {
    direction = (value & Promoted_SIMPLE_LINE_LEFT_OUTER) != 0U ? -1 : 1;
    controller->last_direction = direction;
  ...
  {
    set_output(controller, Promoted_SIMPLE_LINE_WIDE,
               Promoted_SIMPLE_LINE_SLOW_PWM, Promoted_SIMPLE_LINE_SLOW_PWM);
  }
}
