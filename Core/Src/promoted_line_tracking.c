...
typedef enum
{
  Promoted_LINE_RECOVERY_NORMAL = 0U,
  Promoted_LINE_RECOVERY_ACTIVE,
  Promoted_LINE_RECOVERY_SETTLE,
  Promoted_LINE_RECOVERY_STOPPED
} Promoted_LineRecoveryState;

static Promoted_LineRecoveryState recovery_state;
...
{
  if (fast_follow_enabled && recovery_state != Promoted_LINE_RECOVERY_ACTIVE &&
...
static void command_set_pwm(Promoted_LineTrackingCommand *command,
                            int16_t left_pwm,
                            int16_t right_pwm,
                            Promoted_LineTrackingAction action)
{
  if (command == 0) return;
...
static void command_stop(Promoted_LineTrackingCommand *command)
{
  command_set_pwm(command, 0, 0, Promoted_LINE_ACTION_STOP);
}

void Promoted_line_tracking_make_route_command(int8_t direction, int16_t base_speed,
                                      Promoted_LineTrackingCommand *command)
{
  if (!command) return;
  ...
  if (!direction)
  {
    ...
    command_set_pwm(command, cruise, cruise, Promoted_LINE_ACTION_FORWARD);
  }
  else
  {
    command->left_cps = direction < 0 ? 0L : TRACKING_EDGE_OUTER_CPS;
    command->right_cps = direction < 0 ? TRACKING_EDGE_OUTER_CPS : 0L;
    command->action = direction < 0 ? Promoted_LINE_ACTION_LEFT_ADJUST : Promoted_LINE_ACTION_RIGHT_ADJUST;
    command->valid = 1U;
  }
}

void Promoted_line_tracking_make_route_spin_command(int8_t direction, int16_t base_speed,
                                           Promoted_LineTrackingCommand *command)
{
  int16_t turn;
  if (!command) return;
  ...
  command_set_pwm(command,
      ...
      direction < 0 ? Promoted_LINE_ACTION_LEFT_SHARP : Promoted_LINE_ACTION_RIGHT_SHARP);
}

void Promoted_line_tracking_make_slow_arc_command(int8_t direction, int16_t base_speed,
                                         Promoted_LineTrackingCommand *command)
{
  int16_t inner, outer;
  if (!command) return;
  ...
  command_set_pwm(command,
      ...
      direction < 0 ? Promoted_LINE_ACTION_LEFT_ADJUST : Promoted_LINE_ACTION_RIGHT_ADJUST);
}

static void command_set_cps(Promoted_LineTrackingCommand *command,
                            int32_t left_cps,
                            int32_t right_cps,
                            Promoted_LineTrackingAction action)
{
...
static uint8_t command_middle_guard_visible(const Promoted_LineTrackingReading *reading,
                                            Promoted_LineTrackingCommand *command)
{
  ...
  {
    if (reading->x2_black)
      command_set_cps(command, 0L, Promoted_LINE_TRACKING_MIDDLE_GUARD_CPS,
                      Promoted_LINE_ACTION_LEFT_ADJUST);
    else
      command_set_cps(command, TRACKING_MIDDLE_GUARD_INNER_CPS,
                      Promoted_LINE_TRACKING_MIDDLE_GUARD_CPS,
                      Promoted_LINE_ACTION_LEFT_ADJUST);
    return 1U;
  }
  ...
  {
    if (reading->x4_black)
      command_set_cps(command, Promoted_LINE_TRACKING_MIDDLE_GUARD_CPS, 0L,
                      Promoted_LINE_ACTION_RIGHT_ADJUST);
    else
  ...
  command_set_cps(command, Promoted_LINE_TRACKING_MIDDLE_GUARD_CPS,
                  Promoted_LINE_TRACKING_MIDDLE_GUARD_CPS,
                  Promoted_LINE_ACTION_FORWARD);
  return 1U;
}

static void command_middle_guard_spin(Promoted_LineTrackingCommand *command)
{
  int8_t side = Promoted_LineRecovery_GetDirection();
  int32_t left = side < 0 ? -Promoted_LINE_TRACKING_MIDDLE_GUARD_CPS :
                            Promoted_LINE_TRACKING_MIDDLE_GUARD_CPS;
  ...
  command->action = side < 0 ? Promoted_LINE_ACTION_SEARCH_LEFT : Promoted_LINE_ACTION_SEARCH_RIGHT;
  command->valid = 0U;
  DriveBase_PrepareLineTurnAssist(left, -left);
  DriveBase_SetWheelCps(left, left, -left, -left);
}

static void command_visible_adjust(const Promoted_LineTrackingReading *r, Promoted_LineTrackingCommand *command)
{
  ...
  Promoted_LineTrackingAction action = Promoted_LINE_ACTION_FORWARD;
  ...
  if (r->x2_black && !r->x3_black && !r->x4_black)
  {
    ...
    action = Promoted_LINE_ACTION_LEFT_ADJUST;
  }
  else if (r->x4_black && !r->x1_black && !r->x2_black)
  {
    ...
    action = Promoted_LINE_ACTION_RIGHT_ADJUST;
  }
  if (action != Promoted_LINE_ACTION_FORWARD)
  {
    int8_t live_side = action == Promoted_LINE_ACTION_LEFT_ADJUST ? -1 : 1;
  ...
  { left = fast_follow_enabled ? 2400 : TRACKING_SETTLE_INNER_PWM;
    right = fast_follow_enabled ? 2750 : TRACKING_SETTLE_OUTER_PWM; action = Promoted_LINE_ACTION_LEFT_ADJUST; }
  else if (r->x3_black && !r->x1_black)
  { left = fast_follow_enabled ? 2750 : TRACKING_SETTLE_OUTER_PWM;
    right = fast_follow_enabled ? 2400 : TRACKING_SETTLE_INNER_PWM; action = Promoted_LINE_ACTION_RIGHT_ADJUST; }
  command_set_pwm(command, left, right, action);
}
void Promoted_line_tracking_apply_command(const Promoted_LineTrackingCommand *command, int16_t forward_limit_pwm)
{
  Promoted_line_tracking_apply_command_cps(command,
      DriveBase_EquivalentCpsFromPwm(clamp_speed(forward_limit_pwm)));
}

void Promoted_line_tracking_apply_command_cps(const Promoted_LineTrackingCommand *command, int32_t forward_limit_cps)
{
...
static void recovery_stop(Promoted_LineRecoveryStopReason reason)
{
  Promoted_LineRecovery_Stop(reason);
  recovery_state = Promoted_LINE_RECOVERY_STOPPED;
}
...
int8_t Promoted_line_tracking_direction_evidence(const Promoted_LineTrackingReading *r)
{
  if (!r) return 0;
...
static uint8_t reading_mask(const Promoted_LineTrackingReading *r)
{
...
static void update_direction_hint(const Promoted_LineTrackingReading *reading,
                                  uint8_t active_count, uint32_t now)
{
  ...
  if (Promoted_line_tracking_direction_evidence(reading))
  {
...
void Promoted_line_tracking_init(void)
{
...
void Promoted_line_tracking_reset(void)
{
  ...
  recovery_state = Promoted_LINE_RECOVERY_NORMAL;
  recovery_state_started_ms = HAL_GetTick();
}

void Promoted_line_tracking_set_straight_boost(uint8_t enable)
{
...
void Promoted_line_tracking_set_fast_follow(uint8_t enable)
{
...
void Promoted_line_tracking_set_no_line_forward(uint8_t enable)
{
  no_line_forward_enabled = enable != 0U ? 1U : 0U;
}

void Promoted_line_tracking_set_smooth_mode(uint8_t enable)
{
...
void Promoted_line_tracking_set_turn_gain_percent(uint16_t percent)
{
  if (percent < 50U)
  {
    percent = 50U;
...
void Promoted_line_tracking_start_following(void)
{
  Promoted_line_tracking_reset();
  Promoted_line_tracking_set_no_line_forward(0U);
  Promoted_line_tracking_set_smooth_mode(1U);
  Promoted_line_tracking_set_turn_gain_percent(100U);
}

void Promoted_line_tracking_yield_to_route(void)
{
  ...
  Promoted_LineRecovery_Commit();
  Promoted_line_tracking_reset();
}

Promoted_LineTrackingAction Promoted_line_tracking_follow_once(int16_t base_speed, int16_t forward_limit_pwm)
{
  Promoted_LineTrackingReading reading = Promoted_line_tracking_read();
  Promoted_LineTrackingCommand command;
  Promoted_LineTrackingAction action = Promoted_line_tracking_compute(&reading, base_speed, &command);
  Promoted_line_tracking_apply_command(&command, forward_limit_pwm);
  return action;
}

void Promoted_line_tracking_rejoin_from_bypass(uint8_t contact_mask)
{
  Promoted_line_tracking_start_following();
  ...
  recovery_state = Promoted_LINE_RECOVERY_SETTLE;
  recovery_state_started_ms = HAL_GetTick();
}

Promoted_LineTrackingReading Promoted_line_tracking_read(void)
{
  Promoted_LineTrackingReading reading = {0};
...
static uint8_t transverse(const Promoted_LineTrackingReading *r)
{
...
static uint8_t unambiguous_edge(const Promoted_LineTrackingReading *r)
{
  return (r->x2_black && !r->x3_black && !r->x4_black) ||
      (r->x4_black && !r->x1_black && !r->x2_black);
}
static void observe_raw_position(const Promoted_LineTrackingReading *r, uint32_t now)
{
...
static void consume_sampled_evidence(uint32_t through_ms)
{
  ...
  {
    Promoted_LineTrackingReading r = {0};
    uint8_t n;
    ...
    if (transverse(&r))
    {
      if (Promoted_line_tracking_direction_evidence(&r)) update_direction_hint(&r, n, sample.time_ms);
    ...
    if (recovery_state == Promoted_LINE_RECOVERY_ACTIVE)
      Promoted_LineRecovery_ObserveDirection(&r, sample.time_ms);
  }
}
static Promoted_LineTrackingAction Promoted_line_tracking_compute_profile(const Promoted_LineTrackingReading *reading,
                                         int16_t base_speed,
                                         Promoted_LineTrackingCommand *command,
                                         ...
                                         uint8_t current_line_priority)
{
  ...
  {
    return Promoted_LINE_ACTION_STOP;
  }
  ...
  {
    recovery_stop(Promoted_LINE_REC_STOP_DRIVE_FAULT);
    return Promoted_LINE_ACTION_STOP;
  }
  if (recovery_state == Promoted_LINE_RECOVERY_STOPPED) return Promoted_LINE_ACTION_STOP;
  ...
  {
    Promoted_LineRecoveryResult result;
    DriveBaseTelemetry telemetry;

    smooth_filter_valid = smooth_centered_active = 0U;
    smooth_straight_pwm = TRACKING_SMOOTH_STRAIGHT_BASE_PWM;
    update_direction_hint(reading, active_count, now);
    if (recovery_state == Promoted_LINE_RECOVERY_STOPPED)
    {
      command_stop(command);
      return Promoted_LINE_ACTION_STOP;
    }
    if (recovery_state != Promoted_LINE_RECOVERY_ACTIVE)
    {
      ...
      else if (recovery_state == Promoted_LINE_RECOVERY_NORMAL &&
               ambiguous_inner_side != 0 &&
      ...
      if (inner_probe_start)
        Promoted_LineRecovery_BeginAmbiguous(recovery_turn_direction, now);
      else
        Promoted_LineRecovery_Begin(recovery_turn_direction, now);
      ambiguous_inner_side = 0;
      ambiguous_inner_mask = 0U;
      recovery_state = Promoted_LINE_RECOVERY_ACTIVE;
    }

    result = Promoted_LineRecovery_Step(reading, command, now);
    if (result == Promoted_LINE_RECOVERY_FAILED)
    {
      recovery_stop(Promoted_LineRecovery_GetStopReason());
      command_stop(command);
      return Promoted_LINE_ACTION_STOP;
    }
    if (Promoted_LineRecovery_GetDirection() != last_logged_side)
    {
      recovery_turn_direction = Promoted_LineRecovery_GetDirection();
  ...
  {
    if (Promoted_line_tracking_direction_evidence(reading)) update_direction_hint(reading, active_count, now);
  ...
  if ((recovery_state == Promoted_LINE_RECOVERY_NORMAL || recovery_state == Promoted_LINE_RECOVERY_SETTLE) && edge_side)
  {
    smooth_filter_valid = smooth_centered_active = 0U;
    smooth_straight_pwm = TRACKING_SMOOTH_STRAIGHT_BASE_PWM;
  ...
  if (recovery_state == Promoted_LINE_RECOVERY_ACTIVE)
  {
    Promoted_LineRecoveryResult result = fast_follow_enabled ?
        Promoted_LineRecovery_StepRolling(reading, command, now) : Promoted_LineRecovery_Step(reading, command, now);
    if (result != Promoted_LINE_RECOVERY_FAILED && Promoted_LineRecovery_GetDirection() != last_logged_side)
    {
      recovery_turn_direction = Promoted_LineRecovery_GetDirection();
      record_search(now, LINE_SEARCH_CORRECTION);
    }
    if (result == Promoted_LINE_RECOVERY_CAPTURED)
    {
    ...
    else if (result == Promoted_LINE_RECOVERY_FAILED)
    {
      recovery_stop(Promoted_LineRecovery_GetStopReason());
      command_stop(command);
    }
    if (result != Promoted_LINE_RECOVERY_CAPTURED)
    {
      if (result == Promoted_LINE_RECOVERY_BUSY && active_count)
      {
        DriveBaseTelemetry telemetry;
        DriveBase_GetTelemetry(&telemetry);
        if (telemetry.mode != DRIVE_BASE_BRAKING)
        {
          if (fast_follow_enabled && middle_only)
          {
            ...
            command->left_cps = Promoted_LineRecovery_GetDirection() < 0 ? -1800L : 1800L;
            command->right_cps = -command->left_cps;
            command->valid = 1U;
          }
  ...
  if (active_count == 0U)
  {
    ...
    {
      command_set_pwm(command, follow_center_pwm(), follow_center_pwm(), Promoted_LINE_ACTION_FORWARD);
      return command->action;
    }
    if (no_line_forward_enabled != 0U && line_has_been_seen == 0U)
    {
      command_set_pwm(command, base_speed, base_speed, Promoted_LINE_ACTION_FORWARD);
      return Promoted_LINE_ACTION_FORWARD;
    }
    if (predicted_turn_direction != 0 && now - direction_last_seen_ms <=
        (direction_crossing_hold ? TRACKING_CROSS_HINT_MAX_AGE_MS : TRACKING_HINT_MAX_AGE_MS))
    {
    ...
    else if (recovery_state == Promoted_LINE_RECOVERY_NORMAL) recovery_turn_direction = 0;
    ...
    if ((fast_follow_enabled ? Promoted_LineRecovery_StepRolling(reading, command, now) :
         Promoted_LineRecovery_Step(reading, command, now)) == Promoted_LINE_RECOVERY_FAILED)
    {
      recovery_stop(Promoted_LineRecovery_GetStopReason());
      command_stop(command);
    }
  ...
  if (recovery_state == Promoted_LINE_RECOVERY_SETTLE)
  {
    if (fast_follow_enabled)
    {
  ...
  if (settling || hold_slow_profile)
  {
    if (fast_follow_enabled)
    {
      ...
      command->action = reading->x1_black ? Promoted_LINE_ACTION_LEFT_ADJUST : Promoted_LINE_ACTION_RIGHT_ADJUST;
      command->valid = 1U;
      return command->action;
    }
        ...
        line_position > -2 && line_position < 2)
    {
      ...
      if (steering < -TRACKING_SMOOTH_STEER_DEADBAND)
      {
        command_set_pwm(command, left_target, right_target,
                        Promoted_LINE_ACTION_LEFT_ADJUST);
        return Promoted_LINE_ACTION_LEFT_ADJUST;
      }
      if (steering > TRACKING_SMOOTH_STEER_DEADBAND)
      {
        command_set_pwm(command, left_target, right_target,
                        Promoted_LINE_ACTION_RIGHT_ADJUST);
        return Promoted_LINE_ACTION_RIGHT_ADJUST;
      }
...
Promoted_LineTrackingAction Promoted_line_tracking_compute(const Promoted_LineTrackingReading *reading,
                                         int16_t base_speed,
                                         Promoted_LineTrackingCommand *command)
{
  return Promoted_line_tracking_compute_profile(reading, base_speed, command, 0U, 0U);
}

Promoted_LineTrackingAction Promoted_line_tracking_compute_slow(const Promoted_LineTrackingReading *reading,
                                              int16_t base_speed,
                                              Promoted_LineTrackingCommand *command)
{
  return Promoted_line_tracking_compute_profile(reading, base_speed, command, 1U, 0U);
}

Promoted_LineTrackingAction Promoted_line_tracking_compute_arc(const Promoted_LineTrackingReading *reading,
                                              int16_t base_speed,
                                              Promoted_LineTrackingCommand *command)
{
  return Promoted_line_tracking_compute_profile(reading, base_speed, command, 1U, 1U);
}

Promoted_LineTrackingAction Promoted_line_tracking_compute_arc_fallback(const Promoted_LineTrackingReading *reading,
                                                      int16_t base_speed,
                                                      int8_t direction,
                                                      Promoted_LineTrackingCommand *command)
{
  ...
  /* Keep ISR history bounded even through a long white section. This path
     deliberately does not start Promoted_LineRecovery's counter-rotation state. */
  consume_sampled_evidence(now);
  last_observation_ms=now;
  observe_raw_position(reading,now);
  if (reading_mask(reading)!=0U) line_has_been_seen=1U;
  Promoted_line_tracking_make_slow_arc_command(direction,base_speed,command);
  return direction<0 ? Promoted_LINE_ACTION_LEFT_ADJUST : Promoted_LINE_ACTION_RIGHT_ADJUST;
}
