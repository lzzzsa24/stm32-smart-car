...
#include <string.h>
static Promoted_SignTraceRecord records[Promoted_SIGN_TRACE_CAPACITY];
...
uint16_t Promoted_SignTrace_Count(void) { return count; }
uint8_t Promoted_SignTrace_Get(uint16_t index, Promoted_SignTraceRecord *out)
{
  if (!out || index>=count) return 0;
  *out=records[(head+Promoted_SIGN_TRACE_CAPACITY-count+index)%Promoted_SIGN_TRACE_CAPACITY];
  return 1;
}
void Promoted_SignTrace_Record(uint32_t now, uint8_t mask, const Promoted_SignRouteStatus *s,
                      int32_t left_cps, int32_t right_cps, uint8_t line_action,
                      uint8_t control_owner, uint8_t route_active)
{
  Promoted_SignTraceRecord r={0}, last;
  if (!s) return;
  if (frozen && (s->state==Promoted_SIGN_ROUTE_ARMED || s->state==Promoted_SIGN_ROUTE_PROBE ||
                 s->state==Promoted_SIGN_ROUTE_SELECTING)) frozen=0;
  if (frozen) return;
  dumping=0; /* starting motion cancels an incomplete dump */
  if (count && Promoted_SignTrace_Get((uint16_t)(count-1),&last) &&
  ...
  records[head]=r; head=(uint16_t)((head+1)%Promoted_SIGN_TRACE_CAPACITY);
  if(count<Promoted_SIGN_TRACE_CAPACITY) ++count;
  if(s->state==Promoted_SIGN_ROUTE_CANCELLED) frozen=1;
}
void Promoted_SignTrace_Request(uint8_t clear) { request=clear?2U:1U; }
void Promoted_SignTrace_Task(uint8_t stopped)
{
  Promoted_SignTraceRecord r;
  if(!stopped) { dumping=0; return; }
  if(request==2U) { Promoted_SignTrace_Init(); DiagnosticUart_WriteString("STRACE CLEARED\r\n"); return; }
  if(request==1U)
  {
  ...
  if(!dumping) return;
  if(!Promoted_SignTrace_Get(cursor++,&r))
