...
做低速差速寻线，使用角度提前降速，并在接近横线时用底边中心决定原地转向方向。
丢线后按最后预测方向持续原地搜索，直到重新捕获黑线或遥控 `0` 停车；UART
超过 150 ms 没有新合法帧仍会停车。
...
为避免在多交叉轨迹地图上跳到远处黑线，v4 候选只取画面 y=80--239 的近场，
且候选黑块必须延伸到 y>=170；丢线后的重新捕获还需连续两帧确认。
