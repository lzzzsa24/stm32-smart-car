...
切换模式保留、断电或复位恢复40°。达到设定角后沿当时航向直接前进，
不先回正；无出弧时间/距离上限，陀螺仪失效仍前进，须准备好遥控0。
出弧完成要求对应最外侧在出弧后的新采样中先灭后亮；
...
详见 [定版发布说明](docs/releases/RELEASE_V1.2.0.md)。
...
定版固件已完成STM32烧录、完整读回校验和GO；未记录地面测试通过。
本次发布不重新操作STM32或K210，定版标记不代表实物效果保证。
