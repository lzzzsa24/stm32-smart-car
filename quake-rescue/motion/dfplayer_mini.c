...
static volatile uint8_t rx_track_finished;
...
static void receive_byte(uint8_t value)
{
  ...
  /* 记录「当前曲目已播完」，供 Task() 里 loop_current_enabled==0 时停住，
     避免一次性播报结束后 DFPlayer 自动顺序播下一首。 */
  if ((command == DFPLAYER_COMMAND_TRACK_FINISHED_SD ||
       command == DFPLAYER_COMMAND_TRACK_FINISHED_USB) &&
      parameter != 0U)
  {
    rx_track_finished = 1U;
    __DMB();
  }
}
...
void DfPlayerMini_Task(uint32_t now_ms)
{
  ...
  reported_finished = rx_track_finished;
  rx_track_ready = 0U;
  rx_track_finished = 0U;
  HAL_NVIC_EnableIRQ(UART4_IRQn);

  /* 一次性播报已播完（未开循环时 DFPlayer 会自动播下一首）：把软件状态
     置回 STOPPED，并补发一次 STOP 让播放器停在当前曲目，否则会一路顺序
     播完整张 SD 卡。开循环(loop_current_enabled=1)时 DFPlayer 不发 0x3D，
     循环广播不受影响。 */
  if (reported_finished != 0U && loop_current_enabled == 0U)
  {
    playback_state = DFPLAYER_MINI_STOPPED;
    stop_pending = 1U;
  }
